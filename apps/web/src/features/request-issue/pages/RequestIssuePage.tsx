...
import { IssueRequest, IssueRequestType, IssueRequestStatus } from '../../../shared/types';
import { getTodayUtc7String, getNowUtc7String, formatDate, formatDateTime } from '../../../shared/utils/dateUtils';
...
export const RequestIssuePage: React.FC = () => {
  const {
    issueRequests,
    materials,
    batches,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs">
        <div>
          <div className="flex items-center gap-2 text-[#007D3C] text-xs font-bold uppercase tracking-wider mb-1">
            <FileText className="w-4 h-4" /> YÊU CẦU VẬT TƯ SẢN XUẤT (OUT-01 / OUT-02 / OUT-03)
          </div>
          <h1 className="text-xl sm:text-2xl font-extrabold text-slate-900">
            Đăng Ký Đề Nghị Xuất Kho Vật Tư
          </h1>
          <p className="text-xs text-slate-500 mt-1">
            Dành cho Phân xưởng sản xuất, R&D và các Bộ phận sử dụng vật tư: Lập phiếu theo định mức BOM, theo dõi phê duyệt và xác nhận nhận hàng.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          >
            Tiến Độ Đề Nghị Của Đơn Vị ({issueRequests.length})
          </button>
        </div>
      </div>
      ...
      {/* Detail Modal for Requesting Department */}
      {selectedRequest && activeTab === 'requests' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6 border border-slate-200 space-y-4 animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div>
                <h3 className="font-bold text-slate-900 text-base">
                  Chi Tiết Đề Nghị: {selectedRequest.code}
                </h3>
                <span className="text-xs text-slate-500">Đơn vị: {selectedRequest.department}</span>
              </div>
            ...
            <div className="p-3.5 bg-slate-50 rounded-xl border border-slate-200 text-xs space-y-1.5">
              ...
              <div>- Trạng thái: {getStatusBadge(selectedRequest.status)}</div>
            ...
            {/* Danh sách vật tư thực tế từ CSDL MMS1 */}
            {isLoadingDetail ? (
            ...
            <div className="flex justify-end pt-2 border-t border-slate-100">
              <button
                ...
                className="px-5 py-2 text-xs font-semibold bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl cursor-pointer"
              >
                Đóng
              </button>
            </div>
