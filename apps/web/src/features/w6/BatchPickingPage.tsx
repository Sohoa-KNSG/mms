...
export function BatchPickingPage() {
  ...
  return <><PageHeader useCaseId="OUT-07" title={`Soạn theo batch · Phiếu #${requestId}`} description="Batch được sắp FIFO; SP kiểm tra lại vật tư, tồn, vị trí và phần nhu cầu còn lại ngay trong giao dịch." />
    <QueryState isLoading={request.isLoading} error={request.error} isEmpty={!request.data} onRetry={() => void request.refetch()}>{request.data ? <>
      ...
      {currentLine ? <section className="form-card"><h2>Batch cho {currentLine.materialId} · còn {formatQuantity(currentLine.remainingQuantity)} {currentLine.unit ?? ''}</h2><QueryState isLoading={batches.isLoading} error={batches.error} isEmpty={batches.data?.length === 0} onRetry={() => void batches.refetch()}><div className="card-grid">{batches.data?.map((batch) => <button type="button" className={`selection-card ${selectedBatch?.batchId === batch.batchId ? 'selected' : ''}`} key={batch.batchId} onClick={() => { setSelectedBatch(batch); setQuantity(String(Math.min(batch.availableQuantity, currentLine.remainingQuantity))); }}><strong>Batch #{batch.batchId}</strong><span>{batch.locationCode ?? 'Không vị trí'} · {batch.locationName ?? ''}</span><span>{formatQuantity(batch.availableQuantity)} {batch.unit ?? ''}</span><small>Nhập {formatDateTime(batch.receivedAt)}</small></button>)}</div></QueryState>
