---
title: "Đặc tả QC-04 - Đánh giá vật tư"
use_case_id: "QC-04"
version: "1.0"
date: "2026-08-13"
...
| Route React | `/quality/evaluation` |
| Màn hình quyền | `scr_qc_danhgia_vattu`, `scr_qc_info_danhgia` |
...
Kết luận đạt/không đạt theo kết quả kiểm.
...
| Thành công | Hoàn tất đúng luồng 'QC-04', trả dữ liệu/kết quả contract và ghi audit khi có command. |
...
Kết quả mâu thuẫn hoặc thiếu: yêu cầu bổ sung.

### 1.5. Business Rules

| Mã | Quy tắc |
| --- | --- |
| BR-QC-04-01 | User phải có phiên xác thực và quyền màn hình tương ứng. |
| BR-QC-04-02 | Dữ liệu bắt buộc phải được trim và kiểm tra tại API lẫn stored procedure. |
| BR-QC-04-03 | Không tin 'UserId', trạng thái hoặc giá trị suy diễn do client tự gửi. |
| BR-QC-04-04 | Stored procedure là nơi thực thi logic nghiệp vụ và phân quyền dữ liệu. |
| BR-QC-04-05 | Command phải nguyên tử, rollback khi bất kỳ bước nào lỗi. |
| BR-QC-04-06 | Giữ nguyên cấu trúc bảng và mã trạng thái legacy. |
| BR-QC-04-07 | Lỗi nghiệp vụ phải dùng mã ổn định; lỗi ngoài dự kiến phải có 'traceId'. |
| BR-QC-04-08 | React không truy cập bảng SQL trực tiếp. |
...
- Header hiển thị mã 'QC-04' và tên “Đánh giá vật tư”.
...
| Screen | Route |
| --- | --- |
| scr_qc_danhgia_vattu | /quality/evaluation |
| scr_qc_info_danhgia | Theo route registry |

### 3.2. .NET endpoint contract

| Endpoint name | Vai trò |
| --- | --- |
| `QC-04_GetEvaluation` | .NET endpoint đã định danh |
| `QC-04_EvaluateMaterial` | .NET endpoint đã định danh |
...
| --- | --- |
| `api.usp_QC_QC04_EvaluateMaterial_v1` | `database/stored-procedures/w2/api.usp_QC_QC04_EvaluateMaterial_v1.sql` |
| `api.usp_QC_QC04_GetEvaluation_v1` | `database/stored-procedures/w2/api.usp_QC_QC04_GetEvaluation_v1.sql` |

**api.usp_QC_QC04_EvaluateMaterial_v1**

~~~sql
api.usp_QC_QC04_EvaluateMaterial_v1 @UserId nvarchar(50), @InspectionId int, @ReceivingLineId int, @InspectionType nvarchar(50), @InspectedQuantity decimal(19,4), @FailedQuantity decimal(19,4), @OverallResultCode nvarchar(50), @Results api.QcEvaluationItem_v1 READONLY
~~~

**api.usp_QC_QC04_GetEvaluation_v1**

~~~sql
api.usp_QC_QC04_GetEvaluation_v1 @UserId nvarchar(50), @InspectionId int, @ReceivingLineId int = NULL
~~~
...
| --- | --- | --- |
| 51001 | 403 | Không có quyền đánh giá vật tư. |
| 51022 | 422 | Loại kiểm chỉ nhận AQL hoặc 100%. |
| 51022 | 422 | Kết luận chỉ nhận 1 (Đạt), 2 (Không Đạt), 3 (Nhân Nhượng). |
| 51022 | 422 | Số lượng kiểm/không đạt không hợp lệ. |
| 51022 | 422 | Kết quả tiêu chí không hợp lệ. |
| 51022 | 422 | Kết luận Đạt mâu thuẫn với số lượng/kết quả không đạt. |
| 51022 | 422 | Kết luận Không Đạt cần ít nhất một lỗi hoặc số lượng không đạt. |
| 51009 | 409 | Phiếu kiểm/vật tư không còn ở trạng thái có thể đánh giá. |
| 51022 | 422 | Vật tư chưa có mã kiểm. |
| 51022 | 422 | Số lượng kiểm vượt quá số lượng thực nhận. |
| 51022 | 422 | Phải cung cấp đúng và đủ tiêu chí của mã kiểm. |
| 51009 | 409 | Vật tư đã được đánh giá trong phiếu kiểm này. |
| 51009 | 409 | Vật tư đã được cập nhật bởi giao dịch khác. |
...
| --- | --- | --- |
| `vw_danhgia_vattu_qc, vw_ketqua_vattu_qc` | Dữ liệu nghiệp vụ legacy | SP |
| `api.QcEvaluationItem_v1` | Contract API/view/type | SP |
...
| --- | --- | --- |
| AC-QC-04-01 | User có quyền mở use case | Màn hình và dữ liệu đúng phạm vi được hiển thị |
| AC-QC-04-02 | User không có quyền | HTTP 403, không lộ dữ liệu |
| AC-QC-04-03 | Dữ liệu hợp lệ | Hoàn tất đúng luồng chính |
| AC-QC-04-04 | Thiếu dữ liệu bắt buộc | HTTP 400/422, chỉ rõ lỗi |
| AC-QC-04-05 | Đối tượng không tồn tại | HTTP 404, không ghi dở dang |
| AC-QC-04-06 | Dữ liệu đã thay đổi đồng thời | HTTP 409 hoặc kết quả nhất quán |
| AC-QC-04-07 | Lỗi hệ thống | HTTP 500 với 'traceId' |
| AC-QC-04-08 | Kiểm tra audit | User, thời gian và hành động đúng contract |
| AC-QC-04-09 | Kiểm tra phân quyền dữ liệu | Không thấy dữ liệu ngoài phạm vi |
| AC-QC-04-10 | Kiểm tra Power Apps dự phòng | Bảng và trạng thái legacy vẫn tương thích |
...
| Nguồn | Tham chiếu |
| --- | --- |
| Hồ sơ nghiệp vụ | 'HO_SO_TONG_THE_UNG_DUNG_MMS.md' – QC-04 |
| Route registry | 'apps/web/src/app/routeRegistry.ts' |
| API source | apps/api/Modules/Quality/QualityEndpoints.cs |
| SQL source | database/stored-procedures/w2/api.usp_QC_QC04_EvaluateMaterial_v1.sql, database/stored-procedures/w2/api.usp_QC_QC04_GetEvaluation_v1.sql |
