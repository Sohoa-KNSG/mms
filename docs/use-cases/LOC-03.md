---
title: "Đặc tả LOC-03 - Đổi vị trí kệ"
use_case_id: "LOC-03"
version: "1.0"
date: "2026-08-13"
...
| Route React | `/locations/relocate` |
| Màn hình quyền | `scr_luukho_doi_ke` |
...
| Thành công | Hoàn tất đúng luồng 'LOC-03', trả dữ liệu/kết quả contract và ghi audit khi có command. |
...
2. xác nhận nguồn.
3. quét đích.
4. cập nhật và ghi sự kiện..

### 1.4. Ngoại lệ và kiểm soát

Đích không hợp lệ: giữ nguyên vị trí cũ.

### 1.5. Business Rules

| Mã | Quy tắc |
| --- | --- |
| BR-LOC-03-01 | User phải có phiên xác thực và quyền màn hình tương ứng. |
| BR-LOC-03-02 | Dữ liệu bắt buộc phải được trim và kiểm tra tại API lẫn stored procedure. |
| BR-LOC-03-03 | Không tin 'UserId', trạng thái hoặc giá trị suy diễn do client tự gửi. |
| BR-LOC-03-04 | Stored procedure là nơi thực thi logic nghiệp vụ và phân quyền dữ liệu. |
| BR-LOC-03-05 | Command phải nguyên tử, rollback khi bất kỳ bước nào lỗi. |
| BR-LOC-03-06 | Giữ nguyên cấu trúc bảng và mã trạng thái legacy. |
| BR-LOC-03-07 | Lỗi nghiệp vụ phải dùng mã ổn định; lỗi ngoài dự kiến phải có 'traceId'. |
| BR-LOC-03-08 | React không truy cập bảng SQL trực tiếp. |
...
- Header hiển thị mã 'LOC-03' và tên “Đổi vị trí kệ”.
...
| Screen | Route |
| --- | --- |
| scr_luukho_doi_ke | /locations/relocate |

### 3.2. .NET endpoint contract

| Endpoint name | Vai trò |
| --- | --- |
| `LOC-03_GetRelocationWorklist` | .NET endpoint đã định danh |
| `LOC-03_RelocateBatches` | .NET endpoint đã định danh |
...
| --- | --- |
| `api.usp_WMS_LOC03_GetRelocationWorklist_v1` | `database/stored-procedures/w4/api.usp_WMS_LOC03_GetRelocationWorklist_v1.sql` |
| `api.usp_WMS_LOC03_RelocateBatches_v1` | `database/stored-procedures/w4/api.usp_WMS_LOC03_RelocateBatches_v1.sql` |

**api.usp_WMS_LOC03_GetRelocationWorklist_v1**

~~~sql
api.usp_WMS_LOC03_GetRelocationWorklist_v1 @UserId nvarchar(50), @Search nvarchar(200) = NULL, @LocationCode nvarchar(50) = NULL, @Page int = 1, @PageSize int = 50
~~~

**api.usp_WMS_LOC03_RelocateBatches_v1**

~~~sql
api.usp_WMS_LOC03_RelocateBatches_v1 @UserId nvarchar(50), @TargetLocationCode nvarchar(50), @Batches api.BatchLocationItem_v1 READONLY
~~~
...
| --- | --- | --- |
| 51001 | 403 | Không có quyền đổi vị trí kệ. |
| 51002 | 400 | Vị trí đích và batch là bắt buộc. |
| 51004 | 404 | Không tìm thấy vị trí đích. |
...
flowchart TD
    ...
    S1["xác nhận nguồn"]
    S2["quét đích"]
    S3["cập nhật và ghi sự kiện."]
    S0 --> S1
    S1 --> S2
    S2 --> S3
    S3 --> V{"Hợp lệ?"}
    V -- Có --> OK["Hoàn tất LOC-03"]
...
| --- | --- | --- |
| AC-LOC-03-01 | User có quyền mở use case | Màn hình và dữ liệu đúng phạm vi được hiển thị |
| AC-LOC-03-02 | User không có quyền | HTTP 403, không lộ dữ liệu |
| AC-LOC-03-03 | Dữ liệu hợp lệ | Hoàn tất đúng luồng chính |
| AC-LOC-03-04 | Thiếu dữ liệu bắt buộc | HTTP 400/422, chỉ rõ lỗi |
| AC-LOC-03-05 | Đối tượng không tồn tại | HTTP 404, không ghi dở dang |
| AC-LOC-03-06 | Dữ liệu đã thay đổi đồng thời | HTTP 409 hoặc kết quả nhất quán |
| AC-LOC-03-07 | Lỗi hệ thống | HTTP 500 với 'traceId' |
| AC-LOC-03-08 | Kiểm tra audit | User, thời gian và hành động đúng contract |
| AC-LOC-03-09 | Kiểm tra phân quyền dữ liệu | Không thấy dữ liệu ngoài phạm vi |
| AC-LOC-03-10 | Kiểm tra Power Apps dự phòng | Bảng và trạng thái legacy vẫn tương thích |
...
| Nguồn | Tham chiếu |
| --- | --- |
| Hồ sơ nghiệp vụ | 'HO_SO_TONG_THE_UNG_DUNG_MMS.md' – LOC-03 |
...
| SQL source | database/stored-procedures/w4/api.usp_WMS_LOC03_GetRelocationWorklist_v1.sql, database/stored-procedures/w4/api.usp_WMS_LOC03_RelocateBatches_v1.sql |
