---
title: "Đặc tả INV-03 - Tra cứu lịch sử vật tư"
use_case_id: "INV-03"
version: "1.0"
date: "2026-08-13"
...
| Route React | `/inventory/materials` |
| Màn hình quyền | `scr_his_vattu` |
...
Truy vết các batch và giao dịch theo vật tư.
...
| Thành công | Hoàn tất đúng luồng 'INV-03', trả dữ liệu/kết quả contract và ghi audit khi có command. |
...
1. Tìm vật tư.
2. xem tồn/batch.
3. mở lịch sử giao dịch..

### 1.4. Ngoại lệ và kiểm soát

Mã vật tư không hợp lệ: không truy vấn tiếp.

### 1.5. Business Rules

| Mã | Quy tắc |
| --- | --- |
| BR-INV-03-01 | User phải có phiên xác thực và quyền màn hình tương ứng. |
| BR-INV-03-02 | Dữ liệu bắt buộc phải được trim và kiểm tra tại API lẫn stored procedure. |
| BR-INV-03-03 | Không tin 'UserId', trạng thái hoặc giá trị suy diễn do client tự gửi. |
| BR-INV-03-04 | Stored procedure là nơi thực thi logic nghiệp vụ và phân quyền dữ liệu. |
| BR-INV-03-05 | Query không được làm thay đổi dữ liệu nghiệp vụ. |
| BR-INV-03-06 | Giữ nguyên cấu trúc bảng và mã trạng thái legacy. |
| BR-INV-03-07 | Lỗi nghiệp vụ phải dùng mã ổn định; lỗi ngoài dự kiến phải có 'traceId'. |
| BR-INV-03-08 | React không truy cập bảng SQL trực tiếp. |
...
- Header hiển thị mã 'INV-03' và tên “Tra cứu lịch sử vật tư”.
...
| Screen | Route |
| --- | --- |
| scr_his_vattu | /inventory/materials |

### 3.2. .NET endpoint contract

| Endpoint name | Vai trò |
| --- | --- |
| `INV-03_GetMaterialHistory` | .NET endpoint đã định danh |
...
| --- | --- |
| `api.usp_WMS_INV03_GetMaterialHistory_v1` | `database/stored-procedures/w1/api.usp_WMS_INV03_GetMaterialHistory_v1.sql` |

**api.usp_WMS_INV03_GetMaterialHistory_v1**

~~~sql
api.usp_WMS_INV03_GetMaterialHistory_v1 @UserId nvarchar(50), @MaterialId nvarchar(50), @Page int = 1, @PageSize int = 50
~~~
...
| --- | --- | --- |
| 51001 | 403 | Không có quyền xem lịch sử vật tư. |
| 51022 | 422 | Mã vật tư là bắt buộc. |
...
| --- | --- | --- |
| `tbl_dm_vattu, v_transaction_all, v_batch_all` | Dữ liệu nghiệp vụ legacy | SP |
...
sequenceDiagram
    autonumber
    actor User
    participant UI as React INV-03
    participant API as .NET API
    participant SP as SQL Contract
    participant DB as MMS Legacy Tables
    User->>UI: Thực hiện Tra cứu lịch sử vật tư
...
flowchart TD
    S0["Tìm vật tư"]
    S1["xem tồn/batch"]
    S2["mở lịch sử giao dịch."]
    S0 --> S1
    S1 --> S2
    S2 --> V{"Hợp lệ?"}
    V -- Có --> OK["Hoàn tất INV-03"]
...
| --- | --- | --- |
| AC-INV-03-01 | User có quyền mở use case | Màn hình và dữ liệu đúng phạm vi được hiển thị |
| AC-INV-03-02 | User không có quyền | HTTP 403, không lộ dữ liệu |
| AC-INV-03-03 | Dữ liệu hợp lệ | Hoàn tất đúng luồng chính |
| AC-INV-03-04 | Thiếu dữ liệu bắt buộc | HTTP 400/422, chỉ rõ lỗi |
| AC-INV-03-05 | Đối tượng không tồn tại | HTTP 404, không ghi dở dang |
| AC-INV-03-06 | Dữ liệu đã thay đổi đồng thời | HTTP 409 hoặc kết quả nhất quán |
| AC-INV-03-07 | Lỗi hệ thống | HTTP 500 với 'traceId' |
| AC-INV-03-08 | Kiểm tra audit | User, thời gian và hành động đúng contract |
| AC-INV-03-09 | Kiểm tra phân quyền dữ liệu | Không thấy dữ liệu ngoài phạm vi |
| AC-INV-03-10 | Kiểm tra Power Apps dự phòng | Bảng và trạng thái legacy vẫn tương thích |
...
| Nguồn | Tham chiếu |
| --- | --- |
| Hồ sơ nghiệp vụ | 'HO_SO_TONG_THE_UNG_DUNG_MMS.md' – INV-03 |
| Route registry | 'apps/web/src/app/routeRegistry.ts' |
| API source | apps/api/Modules/ReadModels/ReadEndpoints.cs |
| SQL source | database/stored-procedures/w1/api.usp_WMS_INV03_GetMaterialHistory_v1.sql |
