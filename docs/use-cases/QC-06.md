---
title: "Đặc tả QC-06 - In phiếu kiểm"
use_case_id: "QC-06"
version: "1.0"
date: "2026-08-13"
...
| Route React | `/quality/print` |
| Màn hình quyền | `scr_qc_phieukiem_print` |
...
Phát hành bản in kết quả kiểm tra.
...
| Thành công | Hoàn tất đúng luồng 'QC-06', trả dữ liệu/kết quả contract và ghi audit khi có command. |
...
Dữ liệu thiếu: đánh dấu rõ và ngăn phát hành chính thức.

### 1.5. Business Rules

| Mã | Quy tắc |
| --- | --- |
| BR-QC-06-01 | User phải có phiên xác thực và quyền màn hình tương ứng. |
| BR-QC-06-02 | Dữ liệu bắt buộc phải được trim và kiểm tra tại API lẫn stored procedure. |
| BR-QC-06-03 | Không tin 'UserId', trạng thái hoặc giá trị suy diễn do client tự gửi. |
| BR-QC-06-04 | Stored procedure là nơi thực thi logic nghiệp vụ và phân quyền dữ liệu. |
| BR-QC-06-05 | Query không được làm thay đổi dữ liệu nghiệp vụ. |
| BR-QC-06-06 | Giữ nguyên cấu trúc bảng và mã trạng thái legacy. |
| BR-QC-06-07 | Lỗi nghiệp vụ phải dùng mã ổn định; lỗi ngoài dự kiến phải có 'traceId'. |
| BR-QC-06-08 | React không truy cập bảng SQL trực tiếp. |
...
- Header hiển thị mã 'QC-06' và tên “In phiếu kiểm”.
...
| Screen | Route |
| --- | --- |
| scr_qc_phieukiem_print | /quality/print |

### 3.2. .NET endpoint contract

| Endpoint name | Vai trò |
| --- | --- |
| `QC-06_GetInspectionPrintData` | .NET endpoint đã định danh |
...
| --- | --- |
| `api.usp_QC_QC06_GetInspectionPrintData_v1` | `database/stored-procedures/w2/api.usp_QC_QC06_GetInspectionPrintData_v1.sql` |

**api.usp_QC_QC06_GetInspectionPrintData_v1**

~~~sql
api.usp_QC_QC06_GetInspectionPrintData_v1 @UserId nvarchar(50), @InspectionId int
~~~
...
| --- | --- | --- |
| 51001 | 403 | Không có quyền in phiếu kiểm. |
| 51004 | 404 | Không tìm thấy phiếu kiểm. |
| 51022 | 422 | Phiếu kiểm chưa có kết quả. |
| 51022 | 422 | Phiếu kiểm còn vật tư chưa có kết luận. |
...
| --- | --- | --- |
| `vw_phieukiem_group_qc, vw_phieukiem_vattu_qc` | Dữ liệu nghiệp vụ legacy | SP |
...
| --- | --- | --- |
| AC-QC-06-01 | User có quyền mở use case | Màn hình và dữ liệu đúng phạm vi được hiển thị |
| AC-QC-06-02 | User không có quyền | HTTP 403, không lộ dữ liệu |
| AC-QC-06-03 | Dữ liệu hợp lệ | Hoàn tất đúng luồng chính |
| AC-QC-06-04 | Thiếu dữ liệu bắt buộc | HTTP 400/422, chỉ rõ lỗi |
| AC-QC-06-05 | Đối tượng không tồn tại | HTTP 404, không ghi dở dang |
| AC-QC-06-06 | Dữ liệu đã thay đổi đồng thời | HTTP 409 hoặc kết quả nhất quán |
| AC-QC-06-07 | Lỗi hệ thống | HTTP 500 với 'traceId' |
| AC-QC-06-08 | Kiểm tra audit | User, thời gian và hành động đúng contract |
| AC-QC-06-09 | Kiểm tra phân quyền dữ liệu | Không thấy dữ liệu ngoài phạm vi |
| AC-QC-06-10 | Kiểm tra Power Apps dự phòng | Bảng và trạng thái legacy vẫn tương thích |
...
| Nguồn | Tham chiếu |
| --- | --- |
| Hồ sơ nghiệp vụ | 'HO_SO_TONG_THE_UNG_DUNG_MMS.md' – QC-06 |
| Route registry | 'apps/web/src/app/routeRegistry.ts' |
| API source | apps/api/Modules/Quality/QualityEndpoints.cs |
| SQL source | database/stored-procedures/w2/api.usp_QC_QC06_GetInspectionPrintData_v1.sql |
