---
title: "Đặc tả INB-07 - Thực hiện thủ tục nhập kho"
use_case_id: "INB-07"
version: "1.0"
date: "2026-08-13"
...
| Route React | `/receiving/warehouse` |
| Màn hình quyền | `scr_nhapkho_thutuc`, `scr_nhapkho_ql` |
...
Chuyển phiếu nhận đủ điều kiện thành tồn kho.
...
| Thành công | Hoàn tất đúng luồng 'INB-07', trả dữ liệu/kết quả contract và ghi audit khi có command. |
...
2. kiểm tra chi tiết.
3. xác nhận nhập.
4. tạo batch và transaction.
5. cập nhật trạng thái..

### 1.4. Ngoại lệ và kiểm soát

Dữ liệu không đồng nhất: rollback và trả thông báo.

### 1.5. Business Rules

| Mã | Quy tắc |
| --- | --- |
| BR-INB-07-01 | User phải có phiên xác thực và quyền màn hình tương ứng. |
| BR-INB-07-02 | Dữ liệu bắt buộc phải được trim và kiểm tra tại API lẫn stored procedure. |
| BR-INB-07-03 | Không tin 'UserId', trạng thái hoặc giá trị suy diễn do client tự gửi. |
| BR-INB-07-04 | Stored procedure là nơi thực thi logic nghiệp vụ và phân quyền dữ liệu. |
| BR-INB-07-05 | Query không được làm thay đổi dữ liệu nghiệp vụ. |
| BR-INB-07-06 | Giữ nguyên cấu trúc bảng và mã trạng thái legacy. |
| BR-INB-07-07 | Lỗi nghiệp vụ phải dùng mã ổn định; lỗi ngoài dự kiến phải có 'traceId'. |
| BR-INB-07-08 | React không truy cập bảng SQL trực tiếp. |
...
- Header hiển thị mã 'INB-07' và tên “Thực hiện thủ tục nhập kho”.
...
| Screen | Route |
| --- | --- |
| scr_nhapkho_thutuc | /receiving/warehouse |
| scr_nhapkho_ql | /receiving/warehouse |

### 3.2. .NET endpoint contract

| Endpoint name | Vai trò |
| --- | --- |
| `INB-07_GetWarehouseReceiptQueue` | .NET endpoint đã định danh |
| `INB-07_ProcessWarehouseReceipt` | .NET endpoint đã định danh |
...
| --- | --- |
| `api.usp_WMS_INB07_GetWarehouseReceiptQueue_v1` | `database/stored-procedures/w3/api.usp_WMS_INB07_GetWarehouseReceiptQueue_v1.sql` |
| `api.usp_WMS_INB07_ProcessWarehouseReceipt_v1` | `database/stored-procedures/w3/api.usp_WMS_INB07_ProcessWarehouseReceipt_v1.sql` |

**api.usp_WMS_INB07_GetWarehouseReceiptQueue_v1**

~~~sql
api.usp_WMS_INB07_GetWarehouseReceiptQueue_v1 @UserId nvarchar(50), @Search nvarchar(200) = NULL, @ReceiptId int = NULL, @Page int = 1, @PageSize int = 50
~~~

**api.usp_WMS_INB07_ProcessWarehouseReceipt_v1**

~~~sql
api.usp_WMS_INB07_ProcessWarehouseReceipt_v1 @UserId nvarchar(50), @ReceiptId int, @ExpectedStatus nvarchar(50), @Items api.WarehouseReceiptItem_v1 READONLY
~~~
...
| --- | --- | --- |
| 51001 | 403 | Không có quyền thực hiện nhập kho. |
| 51002 | 400 | Phải chọn ít nhất một dòng nhập kho. |
| 51002 | 400 | Số lượng nhập kho phải lớn hơn 0. |
| 51004 | 404 | Không tìm thấy phiếu nhận. |
| 51009 | 409 | Phiếu đã thay đổi. Hãy tải lại. |
| 51022 | 422 | Phiếu phải ở trạng thái đã kiểm trước khi nhập kho. |
| 51022 | 422 | Dòng nhập không hợp lệ, vượt số lượng hoặc chưa đạt QC. |
...
| --- | --- | --- |
| `tbl_batch_inv, tbl_transaction, tbl_phieu_transaction` | Dữ liệu nghiệp vụ legacy | SP |
| `sp_insert_nhapkho` | Dữ liệu nghiệp vụ legacy | SP |
...
| `api.WarehouseReceiptItem_v1` | Contract API/view/type | SP |
...
sequenceDiagram
    autonumber
    actor User
    participant UI as React INB-07
    participant API as .NET API
    participant SP as SQL Contract
    participant DB as MMS Legacy Tables
    User->>UI: Thực hiện Thực hiện thủ tục nhập kho
...
flowchart TD
    S0["Chọn phiếu"]
    S1["kiểm tra chi tiết"]
    S2["xác nhận nhập"]
    S3["tạo batch và transaction"]
    S4["cập nhật trạng thái."]
    S0 --> S1
    S1 --> S2
    S2 --> S3
    S3 --> S4
    S4 --> V{"Hợp lệ?"}
    V -- Có --> OK["Hoàn tất INB-07"]
...
| --- | --- | --- |
| AC-INB-07-01 | User có quyền mở use case | Màn hình và dữ liệu đúng phạm vi được hiển thị |
| AC-INB-07-02 | User không có quyền | HTTP 403, không lộ dữ liệu |
| AC-INB-07-03 | Dữ liệu hợp lệ | Hoàn tất đúng luồng chính |
| AC-INB-07-04 | Thiếu dữ liệu bắt buộc | HTTP 400/422, chỉ rõ lỗi |
| AC-INB-07-05 | Đối tượng không tồn tại | HTTP 404, không ghi dở dang |
| AC-INB-07-06 | Dữ liệu đã thay đổi đồng thời | HTTP 409 hoặc kết quả nhất quán |
| AC-INB-07-07 | Lỗi hệ thống | HTTP 500 với 'traceId' |
| AC-INB-07-08 | Kiểm tra audit | User, thời gian và hành động đúng contract |
| AC-INB-07-09 | Kiểm tra phân quyền dữ liệu | Không thấy dữ liệu ngoài phạm vi |
| AC-INB-07-10 | Kiểm tra Power Apps dự phòng | Bảng và trạng thái legacy vẫn tương thích |
...
| Nguồn | Tham chiếu |
| --- | --- |
| Hồ sơ nghiệp vụ | 'HO_SO_TONG_THE_UNG_DUNG_MMS.md' – INB-07 |
| Route registry | 'apps/web/src/app/routeRegistry.ts' |
| API source | apps/api/Modules/Receiving/ReceivingEndpoints.cs |
| SQL source | database/stored-procedures/w3/api.usp_WMS_INB07_GetWarehouseReceiptQueue_v1.sql, database/stored-procedures/w3/api.usp_WMS_INB07_ProcessWarehouseReceipt_v1.sql |
