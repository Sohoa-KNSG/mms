---
title: "Đặc tả INV-07 - Kiểm kê theo vị trí kệ"
use_case_id: "INV-07"
version: "1.0"
date: "2026-08-13"
...
| Route React | `/inventory/count-location` |
| Màn hình quyền | `scr_kiemke_vitri_ke` |
...
Kiểm tra các batch tại một vị trí.
...
| Thành công | Hoàn tất đúng luồng 'INV-07', trả dữ liệu/kết quả contract và ghi audit khi có command. |
| Thất bại | Không để dữ liệu dở dang; trả lỗi nghiệp vụ hoặc 'traceId'. |

### 1.3. Luồng chính

1. Quét vị trí.
2. tải danh sách batch.
3. quét/đếm.
4. xác nhận chênh lệch..

### 1.4. Ngoại lệ và kiểm soát

Batch ngoài vị trí: đánh dấu ngoại lệ.

### 1.5. Business Rules

| Mã | Quy tắc |
| --- | --- |
| BR-INV-07-01 | User phải có phiên xác thực và quyền màn hình tương ứng. |
| BR-INV-07-02 | Dữ liệu bắt buộc phải được trim và kiểm tra tại API lẫn stored procedure. |
| BR-INV-07-03 | Không tin 'UserId', trạng thái hoặc giá trị suy diễn do client tự gửi. |
| BR-INV-07-04 | Stored procedure là nơi thực thi logic nghiệp vụ và phân quyền dữ liệu. |
| BR-INV-07-05 | Command phải nguyên tử, rollback khi bất kỳ bước nào lỗi. |
| BR-INV-07-06 | Giữ nguyên cấu trúc bảng và mã trạng thái legacy. |
| BR-INV-07-07 | Lỗi nghiệp vụ phải dùng mã ổn định; lỗi ngoài dự kiến phải có 'traceId'. |
| BR-INV-07-08 | React không truy cập bảng SQL trực tiếp. |
...
- Header hiển thị mã 'INV-07' và tên “Kiểm kê theo vị trí kệ”.
...
| Screen | Route |
| --- | --- |
| scr_kiemke_vitri_ke | /inventory/count-location |

### 3.2. .NET endpoint contract

| Endpoint name | Vai trò |
| --- | --- |
| `INV-07_GetLocationCount` | .NET endpoint đã định danh |
| `INV-07_CountLocationBatch` | .NET endpoint đã định danh |
...
| --- | --- |
| `api.usp_WMS_INV07_CountLocationBatch_v1` | `database/stored-procedures/w4/api.usp_WMS_INV07_CountLocationBatch_v1.sql` |
| `api.usp_WMS_INV07_GetLocationCount_v1` | `database/stored-procedures/w4/api.usp_WMS_INV07_GetLocationCount_v1.sql` |

**api.usp_WMS_INV07_CountLocationBatch_v1**

~~~sql
api.usp_WMS_INV07_CountLocationBatch_v1 @UserId nvarchar(50), @LocationCode nvarchar(50), @BatchId int, @ActualQuantity decimal(19,4), @ExpectedQuantity decimal(19,4), @Reason nvarchar(50)
~~~

**api.usp_WMS_INV07_GetLocationCount_v1**

~~~sql
api.usp_WMS_INV07_GetLocationCount_v1 @UserId nvarchar(50), @LocationCode nvarchar(50)
~~~
...
| --- | --- | --- |
| 51001 | 403 | Không có quyền kiểm kê vị trí. |
| 51002 | 400 | Vị trí, số lượng và căn cứ là bắt buộc. |
| 51004 | 404 | Batch không thuộc vị trí được kiểm kê. |
| 51009 | 409 | Tồn batch đã thay đổi. Hãy tải lại. |
| 51022 | 422 | ADJ_UP chưa cấu hình đúng. |
| 51022 | 422 | ADJ_DWN chưa cấu hình đúng. |
| 51004 | 404 | Không tìm thấy vị trí. |
...
| --- | --- | --- |
| `tbl_dm_location, tbl_batch_inv, v_batch_location_all` | Dữ liệu nghiệp vụ legacy | SP |
...
sequenceDiagram
    autonumber
    actor User
    participant UI as React INV-07
    participant API as .NET API
    participant SP as SQL Contract
    participant DB as MMS Legacy Tables
    User->>UI: Thực hiện Kiểm kê theo vị trí kệ
...
flowchart TD
    S0["Quét vị trí"]
    S1["tải danh sách batch"]
    S2["quét/đếm"]
    S3["xác nhận chênh lệch."]
    S0 --> S1
    S1 --> S2
    S2 --> S3
    S3 --> V{"Hợp lệ?"}
    V -- Có --> OK["Hoàn tất INV-07"]
...
| --- | --- | --- |
| AC-INV-07-01 | User có quyền mở use case | Màn hình và dữ liệu đúng phạm vi được hiển thị |
| AC-INV-07-02 | User không có quyền | HTTP 403, không lộ dữ liệu |
| AC-INV-07-03 | Dữ liệu hợp lệ | Hoàn tất đúng luồng chính |
| AC-INV-07-04 | Thiếu dữ liệu bắt buộc | HTTP 400/422, chỉ rõ lỗi |
| AC-INV-07-05 | Đối tượng không tồn tại | HTTP 404, không ghi dở dang |
| AC-INV-07-06 | Dữ liệu đã thay đổi đồng thời | HTTP 409 hoặc kết quả nhất quán |
| AC-INV-07-07 | Lỗi hệ thống | HTTP 500 với 'traceId' |
| AC-INV-07-08 | Kiểm tra audit | User, thời gian và hành động đúng contract |
| AC-INV-07-09 | Kiểm tra phân quyền dữ liệu | Không thấy dữ liệu ngoài phạm vi |
| AC-INV-07-10 | Kiểm tra Power Apps dự phòng | Bảng và trạng thái legacy vẫn tương thích |
...
| Nguồn | Tham chiếu |
| --- | --- |
| Hồ sơ nghiệp vụ | 'HO_SO_TONG_THE_UNG_DUNG_MMS.md' – INV-07 |
...
| SQL source | database/stored-procedures/w4/api.usp_WMS_INV07_CountLocationBatch_v1.sql, database/stored-procedures/w4/api.usp_WMS_INV07_GetLocationCount_v1.sql |
