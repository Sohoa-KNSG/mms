---
title: "Đặc tả INV-01 - Tra cứu tồn kho"
use_case_id: "INV-01"
version: "1.0"
date: "2026-08-13"
...
| Route React | `/inventory/balances` |
| Màn hình quyền | `scr_tonkho_intem` |
...
Xem tồn theo vật tư, batch và vị trí.
...
| Thành công | Hoàn tất đúng luồng 'INV-01', trả dữ liệu/kết quả contract và ghi audit khi có command. |
...
1. Nhập điều kiện.
2. lọc tồn.
3. xem chi tiết batch/vị trí..

### 1.4. Ngoại lệ và kiểm soát

Không có kết quả: phân biệt không tồn và không có quyền.

### 1.5. Business Rules

| Mã | Quy tắc |
| --- | --- |
| BR-INV-01-01 | User phải có phiên xác thực và quyền màn hình tương ứng. |
| BR-INV-01-02 | Dữ liệu bắt buộc phải được trim và kiểm tra tại API lẫn stored procedure. |
| BR-INV-01-03 | Không tin 'UserId', trạng thái hoặc giá trị suy diễn do client tự gửi. |
| BR-INV-01-04 | Stored procedure là nơi thực thi logic nghiệp vụ và phân quyền dữ liệu. |
| BR-INV-01-05 | Query không được làm thay đổi dữ liệu nghiệp vụ. |
| BR-INV-01-06 | Giữ nguyên cấu trúc bảng và mã trạng thái legacy. |
| BR-INV-01-07 | Lỗi nghiệp vụ phải dùng mã ổn định; lỗi ngoài dự kiến phải có 'traceId'. |
| BR-INV-01-08 | React không truy cập bảng SQL trực tiếp. |
...
- Header hiển thị mã 'INV-01' và tên “Tra cứu tồn kho”.
...
| Screen | Route |
| --- | --- |
| scr_tonkho_intem | /inventory/balances |

### 3.2. .NET endpoint contract

| Endpoint name | Vai trò |
| --- | --- |
| `INV-01_GetInventoryBalances` | .NET endpoint đã định danh |
...
| --- | --- |
| `api.usp_WMS_INV01_GetInventoryBalance_v1` | `database/stored-procedures/w1/api.usp_WMS_INV01_GetInventoryBalance_v1.sql` |

**api.usp_WMS_INV01_GetInventoryBalance_v1**

~~~sql
api.usp_WMS_INV01_GetInventoryBalance_v1 @UserId nvarchar(50), @Search nvarchar(200) = NULL, @Page int = 1, @PageSize int = 50
~~~
...
| --- | --- | --- |
| 51001 | 403 | Không có quyền xem tồn kho. |
...
| --- | --- | --- |
| `vw_tong_tonkho, v_ton_he_thong, v_ton_theo_vi_tri` | Dữ liệu nghiệp vụ legacy | SP |
...
flowchart TD
    S0["Nhập điều kiện"]
    S1["lọc tồn"]
    S2["xem chi tiết batch/vị trí."]
    S0 --> S1
    S1 --> S2
    S2 --> V{"Hợp lệ?"}
    V -- Có --> OK["Hoàn tất INV-01"]
...
| --- | --- | --- |
| AC-INV-01-01 | User có quyền mở use case | Màn hình và dữ liệu đúng phạm vi được hiển thị |
| AC-INV-01-02 | User không có quyền | HTTP 403, không lộ dữ liệu |
| AC-INV-01-03 | Dữ liệu hợp lệ | Hoàn tất đúng luồng chính |
| AC-INV-01-04 | Thiếu dữ liệu bắt buộc | HTTP 400/422, chỉ rõ lỗi |
| AC-INV-01-05 | Đối tượng không tồn tại | HTTP 404, không ghi dở dang |
| AC-INV-01-06 | Dữ liệu đã thay đổi đồng thời | HTTP 409 hoặc kết quả nhất quán |
| AC-INV-01-07 | Lỗi hệ thống | HTTP 500 với 'traceId' |
| AC-INV-01-08 | Kiểm tra audit | User, thời gian và hành động đúng contract |
| AC-INV-01-09 | Kiểm tra phân quyền dữ liệu | Không thấy dữ liệu ngoài phạm vi |
| AC-INV-01-10 | Kiểm tra Power Apps dự phòng | Bảng và trạng thái legacy vẫn tương thích |
...
| Nguồn | Tham chiếu |
| --- | --- |
| Hồ sơ nghiệp vụ | 'HO_SO_TONG_THE_UNG_DUNG_MMS.md' – INV-01 |
| Route registry | 'apps/web/src/app/routeRegistry.ts' |
| API source | apps/api/Modules/ReadModels/ReadEndpoints.cs |
| SQL source | database/stored-procedures/w1/api.usp_WMS_INV01_GetInventoryBalance_v1.sql |
