---
title: "Đặc tả INV-06 - Kiểm kê theo batch"
use_case_id: "INV-06"
version: "1.0"
date: "2026-08-13"
...
| Route React | `/inventory/count-batch` |
| Màn hình quyền | `scr_kiemke_batch` |
...
Đối chiếu số lượng thực tế của từng batch.
...
| Thành công | Hoàn tất đúng luồng 'INV-06', trả dữ liệu/kết quả contract và ghi audit khi có command. |
...
Sai batch/vị trí: cảnh báo và yêu cầu xác minh.

### 1.5. Business Rules

| Mã | Quy tắc |
| --- | --- |
| BR-INV-06-01 | User phải có phiên xác thực và quyền màn hình tương ứng. |
| BR-INV-06-02 | Dữ liệu bắt buộc phải được trim và kiểm tra tại API lẫn stored procedure. |
| BR-INV-06-03 | Không tin 'UserId', trạng thái hoặc giá trị suy diễn do client tự gửi. |
| BR-INV-06-04 | Stored procedure là nơi thực thi logic nghiệp vụ và phân quyền dữ liệu. |
| BR-INV-06-05 | Command phải nguyên tử, rollback khi bất kỳ bước nào lỗi. |
| BR-INV-06-06 | Giữ nguyên cấu trúc bảng và mã trạng thái legacy. |
| BR-INV-06-07 | Lỗi nghiệp vụ phải dùng mã ổn định; lỗi ngoài dự kiến phải có 'traceId'. |
| BR-INV-06-08 | React không truy cập bảng SQL trực tiếp. |
...
- Header hiển thị mã 'INV-06' và tên “Kiểm kê theo batch”.
...
| Screen | Route |
| --- | --- |
| scr_kiemke_batch | /inventory/count-batch |

### 3.2. .NET endpoint contract

| Endpoint name | Vai trò |
| --- | --- |
| `INV-06_GetBatchCount` | .NET endpoint đã định danh |
| `INV-06_CountBatch` | .NET endpoint đã định danh |
...
| --- | --- |
| `api.usp_WMS_INV06_CountBatch_v1` | `database/stored-procedures/w4/api.usp_WMS_INV06_CountBatch_v1.sql` |
| `api.usp_WMS_INV06_GetBatchCount_v1` | `database/stored-procedures/w4/api.usp_WMS_INV06_GetBatchCount_v1.sql` |

**api.usp_WMS_INV06_CountBatch_v1**

~~~sql
api.usp_WMS_INV06_CountBatch_v1 @UserId nvarchar(50), @BatchId int, @ActualQuantity decimal(19,4), @ExpectedQuantity decimal(19,4), @Reason nvarchar(50)
~~~

**api.usp_WMS_INV06_GetBatchCount_v1**

~~~sql
api.usp_WMS_INV06_GetBatchCount_v1 @UserId nvarchar(50), @BatchId int
~~~
...
| --- | --- | --- |
| 51001 | 403 | Không có quyền kiểm kê batch. |
| 51002 | 400 | Số lượng thực tế và căn cứ kiểm kê là bắt buộc. |
...
| --- | --- | --- |
| `tbl_batch_inv` | Dữ liệu nghiệp vụ legacy | SP |
| `sp_kiemke_batch` | Dữ liệu nghiệp vụ legacy | SP |
...
| --- | --- | --- |
| AC-INV-06-01 | User có quyền mở use case | Màn hình và dữ liệu đúng phạm vi được hiển thị |
| AC-INV-06-02 | User không có quyền | HTTP 403, không lộ dữ liệu |
| AC-INV-06-03 | Dữ liệu hợp lệ | Hoàn tất đúng luồng chính |
| AC-INV-06-04 | Thiếu dữ liệu bắt buộc | HTTP 400/422, chỉ rõ lỗi |
| AC-INV-06-05 | Đối tượng không tồn tại | HTTP 404, không ghi dở dang |
| AC-INV-06-06 | Dữ liệu đã thay đổi đồng thời | HTTP 409 hoặc kết quả nhất quán |
| AC-INV-06-07 | Lỗi hệ thống | HTTP 500 với 'traceId' |
| AC-INV-06-08 | Kiểm tra audit | User, thời gian và hành động đúng contract |
| AC-INV-06-09 | Kiểm tra phân quyền dữ liệu | Không thấy dữ liệu ngoài phạm vi |
| AC-INV-06-10 | Kiểm tra Power Apps dự phòng | Bảng và trạng thái legacy vẫn tương thích |
...
| Nguồn | Tham chiếu |
| --- | --- |
| Hồ sơ nghiệp vụ | 'HO_SO_TONG_THE_UNG_DUNG_MMS.md' – INV-06 |
...
| SQL source | database/stored-procedures/w4/api.usp_WMS_INV06_CountBatch_v1.sql, database/stored-procedures/w4/api.usp_WMS_INV06_GetBatchCount_v1.sql |
