---
title: "Đặc tả INB-04 - Tra cứu nhật ký nhận hàng"
use_case_id: "INB-04"
version: "1.0"
date: "2026-08-13"
...
| Route React | `/receiving/log` |
| Màn hình quyền | `scr_tam_nhanhang_log`, `scr_nhanhang_log` |
...
Theo dõi lịch sử và trạng thái phiếu nhận.
...
| Thành công | Hoàn tất đúng luồng 'INB-04', trả dữ liệu/kết quả contract và ghi audit khi có command. |
| Thất bại | Không để dữ liệu dở dang; trả lỗi nghiệp vụ hoặc 'traceId'. |

### 1.3. Luồng chính

1. Lọc theo ngày/PO/trạng thái.
2. xem danh sách.
3. mở chi tiết..

### 1.4. Ngoại lệ và kiểm soát

Không có dữ liệu phù hợp: hiển thị empty state.

### 1.5. Business Rules

| Mã | Quy tắc |
| --- | --- |
| BR-INB-04-01 | User phải có phiên xác thực và quyền màn hình tương ứng. |
| BR-INB-04-02 | Dữ liệu bắt buộc phải được trim và kiểm tra tại API lẫn stored procedure. |
| BR-INB-04-03 | Không tin 'UserId', trạng thái hoặc giá trị suy diễn do client tự gửi. |
| BR-INB-04-04 | Stored procedure là nơi thực thi logic nghiệp vụ và phân quyền dữ liệu. |
| BR-INB-04-05 | Query không được làm thay đổi dữ liệu nghiệp vụ. |
| BR-INB-04-06 | Giữ nguyên cấu trúc bảng và mã trạng thái legacy. |
| BR-INB-04-07 | Lỗi nghiệp vụ phải dùng mã ổn định; lỗi ngoài dự kiến phải có 'traceId'. |
| BR-INB-04-08 | React không truy cập bảng SQL trực tiếp. |
...
- Header hiển thị mã 'INB-04' và tên “Tra cứu nhật ký nhận hàng”.
...
| Screen | Route |
| --- | --- |
| scr_tam_nhanhang_log | /receiving/log |
| scr_nhanhang_log | /receiving/log |

### 3.2. .NET endpoint contract

| Endpoint name | Vai trò |
| --- | --- |
| `INB-04_GetReceiptLog` | .NET endpoint đã định danh |
...
| --- | --- |
| `api.usp_WMS_INB04_GetReceiptLog_v1` | `database/stored-procedures/w1/api.usp_WMS_INB04_GetReceiptLog_v1.sql` |

**api.usp_WMS_INB04_GetReceiptLog_v1**

~~~sql
api.usp_WMS_INB04_GetReceiptLog_v1 @UserId nvarchar(50), @Search nvarchar(200) = NULL, @Page int = 1, @PageSize int = 50
~~~
...
| --- | --- | --- |
| 51001 | 403 | Không có quyền xem nhật ký nhận hàng. |
...
| --- | --- | --- |
| `vw_status_phieu_nhanhang, tbl_his_chitiet_nhanhang` | Dữ liệu nghiệp vụ legacy | SP |
...
sequenceDiagram
    autonumber
    actor User
    participant UI as React INB-04
    participant API as .NET API
    participant SP as SQL Contract
    participant DB as MMS Legacy Tables
    User->>UI: Thực hiện Tra cứu nhật ký nhận hàng
...
~~~mermaid
flowchart TD
    S0["Lọc theo ngày/PO/trạng thái"]
    S1["xem danh sách"]
    S2["mở chi tiết."]
    S0 --> S1
    S1 --> S2
    S2 --> V{"Hợp lệ?"}
    V -- Có --> OK["Hoàn tất INB-04"]
...
| --- | --- | --- |
| AC-INB-04-01 | User có quyền mở use case | Màn hình và dữ liệu đúng phạm vi được hiển thị |
| AC-INB-04-02 | User không có quyền | HTTP 403, không lộ dữ liệu |
| AC-INB-04-03 | Dữ liệu hợp lệ | Hoàn tất đúng luồng chính |
| AC-INB-04-04 | Thiếu dữ liệu bắt buộc | HTTP 400/422, chỉ rõ lỗi |
| AC-INB-04-05 | Đối tượng không tồn tại | HTTP 404, không ghi dở dang |
| AC-INB-04-06 | Dữ liệu đã thay đổi đồng thời | HTTP 409 hoặc kết quả nhất quán |
| AC-INB-04-07 | Lỗi hệ thống | HTTP 500 với 'traceId' |
| AC-INB-04-08 | Kiểm tra audit | User, thời gian và hành động đúng contract |
| AC-INB-04-09 | Kiểm tra phân quyền dữ liệu | Không thấy dữ liệu ngoài phạm vi |
| AC-INB-04-10 | Kiểm tra Power Apps dự phòng | Bảng và trạng thái legacy vẫn tương thích |
...
| Nguồn | Tham chiếu |
| --- | --- |
| Hồ sơ nghiệp vụ | 'HO_SO_TONG_THE_UNG_DUNG_MMS.md' – INB-04 |
| Route registry | 'apps/web/src/app/routeRegistry.ts' |
| API source | apps/api/Modules/ReadModels/ReadEndpoints.cs |
| SQL source | database/stored-procedures/w1/api.usp_WMS_INB04_GetReceiptLog_v1.sql |
