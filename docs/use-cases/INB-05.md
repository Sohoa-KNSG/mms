---
title: "Đặc tả INB-05 - Cập nhật nhận hàng theo PO"
use_case_id: "INB-05"
version: "1.0"
date: "2026-08-13"
...
| Route React | `/receiving/attach-po` |
| Màn hình quyền | `scr_nhapkho_update_po` |
...
Điều chỉnh chi tiết PO trước khi nhập kho.
...
| Thành công | Hoàn tất đúng luồng 'INB-05', trả dữ liệu/kết quả contract và ghi audit khi có command. |
...
Không khớp PO hoặc vượt số lượng: thông báo lỗi.

### 1.5. Business Rules

| Mã | Quy tắc |
| --- | --- |
| BR-INB-05-01 | User phải có phiên xác thực và quyền màn hình tương ứng. |
| BR-INB-05-02 | Dữ liệu bắt buộc phải được trim và kiểm tra tại API lẫn stored procedure. |
| BR-INB-05-03 | Không tin 'UserId', trạng thái hoặc giá trị suy diễn do client tự gửi. |
| BR-INB-05-04 | Stored procedure là nơi thực thi logic nghiệp vụ và phân quyền dữ liệu. |
| BR-INB-05-05 | Query không được làm thay đổi dữ liệu nghiệp vụ. |
| BR-INB-05-06 | Giữ nguyên cấu trúc bảng và mã trạng thái legacy. |
| BR-INB-05-07 | Lỗi nghiệp vụ phải dùng mã ổn định; lỗi ngoài dự kiến phải có 'traceId'. |
| BR-INB-05-08 | React không truy cập bảng SQL trực tiếp. |
...
- Header hiển thị mã 'INB-05' và tên “Cập nhật nhận hàng theo PO”.
...
| Screen | Route |
| --- | --- |
| scr_nhapkho_update_po | /receiving/attach-po |

### 3.2. .NET endpoint contract

| Endpoint name | Vai trò |
| --- | --- |
| `INB-05_GetUnmatchedReceipts` | .NET endpoint đã định danh |
| `INB-05_AttachPurchaseOrder` | .NET endpoint đã định danh |
...
| --- | --- |
| `api.usp_WMS_INB05_AttachPurchaseOrder_v1` | `database/stored-procedures/w3/api.usp_WMS_INB05_AttachPurchaseOrder_v1.sql` |
| `api.usp_WMS_INB05_GetUnmatchedReceipts_v1` | `database/stored-procedures/w3/api.usp_WMS_INB05_GetUnmatchedReceipts_v1.sql` |

**api.usp_WMS_INB05_AttachPurchaseOrder_v1**

~~~sql
api.usp_WMS_INB05_AttachPurchaseOrder_v1 @UserId nvarchar(50), @ReceiptId int, @PurchaseOrder nvarchar(50), @ExpectedStatus nvarchar(50), @Assignments api.ReceiptPoAssignmentItem_v1 READONLY
~~~

**api.usp_WMS_INB05_GetUnmatchedReceipts_v1**

~~~sql
api.usp_WMS_INB05_GetUnmatchedReceipts_v1 @UserId nvarchar(50), @Search nvarchar(200) = NULL, @Page int = 1, @PageSize int = 50
~~~
...
| --- | --- | --- |
| 51001 | 403 | Không có quyền cập nhật PO cho phiếu nhận. |
| 51002 | 400 | PO và ánh xạ dòng nhận là bắt buộc. |
...
| 51004 | 404 | Không tìm thấy PO. |
| 51022 | 422 | Mọi dòng của phiếu phải được ánh xạ vào PO. |
...
sequenceDiagram
    autonumber
    actor User
    participant UI as React INB-05
    participant API as .NET API
    participant SP as SQL Contract
    participant DB as MMS Legacy Tables
    User->>UI: Thực hiện Cập nhật nhận hàng theo PO
...
flowchart TD
    S0["Chọn phiếu"]
    S1["cập nhật dòng PO"]
    S2["kiểm tra số lượng"]
    S3["lưu."]
    S0 --> S1
    S1 --> S2
    S2 --> S3
    S3 --> V{"Hợp lệ?"}
    V -- Có --> OK["Hoàn tất INB-05"]
...
| --- | --- | --- |
| AC-INB-05-01 | User có quyền mở use case | Màn hình và dữ liệu đúng phạm vi được hiển thị |
| AC-INB-05-02 | User không có quyền | HTTP 403, không lộ dữ liệu |
| AC-INB-05-03 | Dữ liệu hợp lệ | Hoàn tất đúng luồng chính |
| AC-INB-05-04 | Thiếu dữ liệu bắt buộc | HTTP 400/422, chỉ rõ lỗi |
| AC-INB-05-05 | Đối tượng không tồn tại | HTTP 404, không ghi dở dang |
| AC-INB-05-06 | Dữ liệu đã thay đổi đồng thời | HTTP 409 hoặc kết quả nhất quán |
| AC-INB-05-07 | Lỗi hệ thống | HTTP 500 với 'traceId' |
| AC-INB-05-08 | Kiểm tra audit | User, thời gian và hành động đúng contract |
| AC-INB-05-09 | Kiểm tra phân quyền dữ liệu | Không thấy dữ liệu ngoài phạm vi |
| AC-INB-05-10 | Kiểm tra Power Apps dự phòng | Bảng và trạng thái legacy vẫn tương thích |
...
| Nguồn | Tham chiếu |
| --- | --- |
| Hồ sơ nghiệp vụ | 'HO_SO_TONG_THE_UNG_DUNG_MMS.md' – INB-05 |
| Route registry | 'apps/web/src/app/routeRegistry.ts' |
| API source | apps/api/Modules/Receiving/ReceivingEndpoints.cs |
| SQL source | database/stored-procedures/w3/api.usp_WMS_INB05_AttachPurchaseOrder_v1.sql, database/stored-procedures/w3/api.usp_WMS_INB05_GetUnmatchedReceipts_v1.sql |
