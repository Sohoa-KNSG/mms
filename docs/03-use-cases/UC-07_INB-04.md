# UC-07 (INB-04) — ĐỀ XUẤT VỊ TRÍ Ô KỆ NHẬP KHO (PUTAWAY RECOMMENDATION)

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-07 (INB-04)` |
| Use Case Name | Đề Xuất Vị Trí Ô Kệ Nhập Kho (Putaway Recommendation) |
| Module | `WMS / Putaway Algorithm` |
...
Hệ thống tự động phân tích 540 ô kệ, đối chiếu quy tắc lưu trữ theo Nhóm SKU, tải trọng tối đa của kệ, thể tích chứa, kệ đang chứa cùng SKU (gom hàng) và độ cao tầng kệ (nặng tầng thấp, nhẹ tầng cao) để đưa ra vị trí Ô kệ đề xuất tối ưu nhất.

### 1.2. Primary Actor
- **Hệ thống MMS WMS / Nhân viên cất hàng**
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`PutawayRecommendationModal.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_WMS_INB04_GetPutawayRecommendation_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền và chứng từ ở trạng thái `LABELED`.  
**When** Gửi yêu cầu tiếp nhận / cất kệ / nhập kho thành công.  
**Then** Trả 200 OK, CSDL chuyển sang `RECOMMENDED_LOCATION`, ghi Sổ Cái Kép và phát âm thanh phản hồi.
...
### 6.1. Frontend — React (`PutawayRecommendationModal.tsx`)
...
- **Stored Procedure:** `api.usp_WMS_INB04_GetPutawayRecommendation_v1`
...
|---|:---:|:---:|:---:|:---:|---|
| `dbo.tbl_dm_vitri_khe` | **X** | **X** | **X** | - | Bảng thực thể chính xử lý nhập kho / trả hàng |
...
sequenceDiagram
    autonumber
    actor User as Hệ thống MMS WMS / Nhân viên cất hàng
    participant UI as React UI (PutawayRecommendationModal.tsx)
    ...
    API->>DB: 5. EXEC api.usp_WMS_INB04_GetPutawayRecommendation_v1 @UserId, @Params
    
    activate DB
    Note over DB: BƯỚC 1: SET XACT_ABORT ON & Kiểm tra quyền
    Note over DB: BƯỚC 2: BEGIN TRANSACTION & Khóa dữ liệu mục tiêu (UPDLOCK)
    Note over DB: BƯỚC 3: Kiểm tra điều kiện nghiệp vụ (LABELED)
...
```mermaid
flowchart TD
    User["Hệ thống MMS WMS / Nhân viên cất hàng"]
    ReactUI["React UI (PutawayRecommendationModal.tsx)"]
    BackendAPI["Backend API (.NET 8)"]
    AuthCheck{"Token hợp lệ & Có quyền?"}
    StatusCheck{"Trạng thái hợp lệ (LABELED)?"}
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_INB04_GetPutawayRecommendation_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật RECOMMENDED_LOCATION"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `RECOMMENDED_LOCATION` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_WMS_INB04_GetPutawayRecommendation_v1` chuẩn Transaction.
