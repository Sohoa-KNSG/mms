# UC-17 (INV-01) — TRA CỨU TỒN KHO TỔNG HỢP THEO MÃ VẬT TƯ (SKU)

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-17 (INV-01)` |
| Use Case Name | Tra Cứu Tồn Kho Tổng Hợp Theo Mã Vật Tư (SKU) |
| Module | `WMS / Inventory Inquiry` |
...
Cho phép tra cứu nhanh số lượng tồn kho tổng hợp của toàn bộ 17,476 SKU vật tư trong nhà máy. Hệ thống tổng hợp realtime: Tổng tồn vật lý, Tồn khả dụng (QC Pass & On Rack), Tồn giữ chỗ (Reserved) và đối chiếu với hạn mức an toàn Min/Max.

### 1.2. Primary Actor
- **Thủ kho / Kế toán kho / Kế hoạch**
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`InventorySkuTab.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_WMS_INV01_GetStockBySku_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền và dữ liệu ở trạng thái `STORED`.  
**When** Gửi yêu cầu thực hiện thao tác.  
**Then** Trả 200 OK, CSDL chuyển sang `AVAILABLE`, ghi Sổ Cái Kép và phát âm thanh phản hồi.
...
### 6.1. Frontend — React (`InventorySkuTab.tsx`)
...
- **Stored Procedure:** `api.usp_WMS_INV01_GetStockBySku_v1`
...
sequenceDiagram
    autonumber
    actor User as Thủ kho / Kế toán kho / Kế hoạch
    participant UI as React UI (InventorySkuTab.tsx)
    ...
    API->>DB: 5. EXEC api.usp_WMS_INV01_GetStockBySku_v1 @UserId, @Params
...
```mermaid
flowchart TD
    User["Thủ kho / Kế toán kho / Kế hoạch"]
    ReactUI["React UI (InventorySkuTab.tsx)"]
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_INV01_GetStockBySku_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật AVAILABLE"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `AVAILABLE` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_WMS_INV01_GetStockBySku_v1` chuẩn Transaction.
