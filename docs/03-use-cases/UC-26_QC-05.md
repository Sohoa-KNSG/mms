# UC-26 (QC-05) — IN TEM NHÃN TRẠNG THÁI CHẤT LƯỢNG (TEM QC PASS / TEM CẢNH BÁO)

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-26 (QC-05)` |
| Use Case Name | In Tem Nhãn Trạng Thái Chất Lượng (Tem QC Pass / Tem Cảnh Báo) |
| Module | `WMS / QC Labeling` |
...
Tự động gửi lệnh in Tem kiểm định chất lượng (Tem xanh lá QC PASS kèm ngày kiểm và mã KCS, hoặc Tem đỏ QC REJECT - CẤM SỬ DỤNG) dán đè lên thùng hàng để phân biệt trực quan tại kho.
...
| `BR-${doc.id.replace(/[^a-zA-Z0-9]/g, '')}-03` | Status Gate | Dữ liệu mục tiêu phải ở trạng thái hợp lệ (`DECIDED`). | `409 Conflict` |
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`QcLabelPrintModal.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_WMS_QC05_PrintQcLabel_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền và dữ liệu ở trạng thái `DECIDED`.  
**When** Gửi yêu cầu thực hiện thao tác thành công.  
**Then** Trả 200 OK, CSDL chuyển sang `QC_LABELED`, ghi Audit Log và phát âm thanh phản hồi.
...
sequenceDiagram
    ...
    participant UI as React UI (QcLabelPrintModal.tsx)
...
flowchart TD
    ...
    ReactUI["React UI (QcLabelPrintModal.tsx)"]
    BackendAPI["Backend API (.NET 8)"]
    AuthCheck{"Token hợp lệ & Có quyền?"}
    StatusCheck{"Trạng thái hợp lệ (DECIDED)?"}
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_QC05_PrintQcLabel_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật QC_LABELED"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `QC_LABELED` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_WMS_QC05_PrintQcLabel_v1` chuẩn Transaction.
