# UC-13 (QC-03) — NHẬP KẾT QUẢ ĐO LƯỜNG & GHI NHẬN LỖI KỸ THUẬT (DEFECT RECORDING)

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-13 (QC-03)` |
| Use Case Name | Nhập Kết Quả Đo Lường & Ghi Nhận Lỗi Kỹ Thuật (Defect Recording) |
| Module | `WMS / QC Measurements` |
...
Cung cấp form nhập liệu đo lường kỹ thuật cho các mẫu thử nghiệm: Nhập thông số thực tế, so khớp tự động với dung sai cho phép, đếm số lỗi phát hiện phân loại theo mức độ (Critical, Major, Minor) và đính kèm ảnh chụp khuyết tật.
...
| `BR-${doc.id.replace(/[^a-zA-Z0-9]/g, '')}-03` | Status Gate | Dữ liệu mục tiêu phải ở trạng thái hợp lệ (`IN_INSPECTION`). | `409 Conflict` |
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`QcMeasurementEntryPage.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_WMS_QC03_RecordMeasurements_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền và dữ liệu ở trạng thái `IN_INSPECTION`.  
**When** Gửi yêu cầu thực hiện thao tác thành công.  
**Then** Trả 200 OK, CSDL chuyển sang `MEASURED`, ghi Audit Log và phát âm thanh phản hồi.
...
|---|:---:|:---:|:---:|:---:|---|
| `dbo.tbl_qc_measurements` | **X** | **X** | **X** | - | Bảng thực thể chính xử lý nghiệp vụ |
...
sequenceDiagram
    autonumber
    actor User as Nhân viên KCS / QC
    participant UI as React UI (QcMeasurementEntryPage.tsx)
    ...
    API->>DB: 5. EXEC api.usp_WMS_QC03_RecordMeasurements_v1 @UserId, @Params
    
    activate DB
    Note over DB: BƯỚC 1: SET XACT_ABORT ON & Kiểm tra quyền
    Note over DB: BƯỚC 2: BEGIN TRANSACTION & Khóa dữ liệu mục tiêu (UPDLOCK)
    Note over DB: BƯỚC 3: Kiểm tra điều kiện nghiệp vụ (IN_INSPECTION)
...
```mermaid
flowchart TD
    User["Nhân viên KCS / QC"]
    ReactUI["React UI (QcMeasurementEntryPage.tsx)"]
    BackendAPI["Backend API (.NET 8)"]
    AuthCheck{"Token hợp lệ & Có quyền?"}
    StatusCheck{"Trạng thái hợp lệ (IN_INSPECTION)?"}
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_QC03_RecordMeasurements_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật MEASURED"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `MEASURED` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_WMS_QC03_RecordMeasurements_v1` chuẩn Transaction.
