# UC-11 (LOC-02) — SƠ ĐỒ TRỰC QUAN 2D MẶT ĐỨNG KHO THEO Ô KỆ

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-11 (LOC-02)` |
| Use Case Name | Sơ Đồ Trực Quan 2D Mặt Đứng Kho Theo Ô Kệ |
| Module | `WMS / 2D Elevation Map` |
...
Trực quan hóa toàn bộ 540 ô kệ thành bản đồ 2D mặt đứng tương tác theo từng Dãy kệ theo mã màu heatmap: Ô trống (Xanh lá), Ô chứa 50-80% (Vàng), Ô đầy tải 100% (Đỏ), Ô bị khóa (Xám). Click vào ô để xem danh sách Lô bên trong.

### 1.2. Primary Actor
- **Thủ kho / Ban Quản Đốc**
...
| `BR-${doc.id.replace(/[^a-zA-Z0-9]/g, '')}-03` | Status Gate | Dữ liệu mục tiêu phải ở trạng thái hợp lệ (`MAP_LOADED`). | `409 Conflict` |
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`WarehouseElevationMapPage.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_WMS_LOC02_GetElevationMap_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền và dữ liệu ở trạng thái `MAP_LOADED`.  
**When** Gửi yêu cầu thực hiện thao tác thành công.  
**Then** Trả 200 OK, CSDL chuyển sang `VISUALIZED`, ghi Audit Log và phát âm thanh phản hồi.
...
sequenceDiagram
    autonumber
    actor User as Thủ kho / Ban Quản Đốc
    participant UI as React UI (WarehouseElevationMapPage.tsx)
    ...
    API->>DB: 5. EXEC api.usp_WMS_LOC02_GetElevationMap_v1 @UserId, @Params
    
    activate DB
    Note over DB: BƯỚC 1: SET XACT_ABORT ON & Kiểm tra quyền
    Note over DB: BƯỚC 2: BEGIN TRANSACTION & Khóa dữ liệu mục tiêu (UPDLOCK)
    Note over DB: BƯỚC 3: Kiểm tra điều kiện nghiệp vụ (MAP_LOADED)
...
```mermaid
flowchart TD
    User["Thủ kho / Ban Quản Đốc"]
    ReactUI["React UI (WarehouseElevationMapPage.tsx)"]
    BackendAPI["Backend API (.NET 8)"]
    AuthCheck{"Token hợp lệ & Có quyền?"}
    StatusCheck{"Trạng thái hợp lệ (MAP_LOADED)?"}
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_LOC02_GetElevationMap_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật VISUALIZED"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `VISUALIZED` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_WMS_LOC02_GetElevationMap_v1` chuẩn Transaction.
