---
title: "Đặc tả LOC-02 - Đưa batch lên kệ"
use_case_id: "LOC-02"
version: "1.0"
date: "2026-08-13"
...
| Route React | `/locations/put-away` |
| Màn hình quyền | `scr_luukho_len_ke`, `scr_luukho_ql` |
...
| Thành công | Hoàn tất đúng luồng 'LOC-02', trả dữ liệu/kết quả contract và ghi audit khi có command. |
...
1. Quét batch.
2. quét vị trí.
3. kiểm tra sức chứa/quy tắc.
4. xác nhận..

### 1.4. Ngoại lệ và kiểm soát

Sai vị trí hoặc batch: không cập nhật.

### 1.5. Business Rules

| Mã | Quy tắc |
| --- | --- |
| BR-LOC-02-01 | User phải có phiên xác thực và quyền màn hình tương ứng. |
| BR-LOC-02-02 | Dữ liệu bắt buộc phải được trim và kiểm tra tại API lẫn stored procedure. |
| BR-LOC-02-03 | Không tin 'UserId', trạng thái hoặc giá trị suy diễn do client tự gửi. |
| BR-LOC-02-04 | Stored procedure là nơi thực thi logic nghiệp vụ và phân quyền dữ liệu. |
| BR-LOC-02-05 | Command phải nguyên tử, rollback khi bất kỳ bước nào lỗi. |
| BR-LOC-02-06 | Giữ nguyên cấu trúc bảng và mã trạng thái legacy. |
| BR-LOC-02-07 | Lỗi nghiệp vụ phải dùng mã ổn định; lỗi ngoài dự kiến phải có 'traceId'. |
| BR-LOC-02-08 | React không truy cập bảng SQL trực tiếp. |
...
- Header hiển thị mã 'LOC-02' và tên “Đưa batch lên kệ”.
...
| Screen | Route |
| --- | --- |
| scr_luukho_len_ke | /locations/put-away |
| scr_luukho_ql | /locations/put-away |

### 3.2. .NET endpoint contract

| Endpoint name | Vai trò |
| --- | --- |
| `LOC-02_GetPutAwayWorklist` | .NET endpoint đã định danh |
| `LOC-02_PutAwayBatches` | .NET endpoint đã định danh |
...
| --- | --- |
| `api.usp_WMS_LOC02_GetPutAwayWorklist_v1` | `database/stored-procedures/w4/api.usp_WMS_LOC02_GetPutAwayWorklist_v1.sql` |
| `api.usp_WMS_LOC02_PutAwayBatches_v1` | `database/stored-procedures/w4/api.usp_WMS_LOC02_PutAwayBatches_v1.sql` |

**api.usp_WMS_LOC02_GetPutAwayWorklist_v1**

~~~sql
api.usp_WMS_LOC02_GetPutAwayWorklist_v1 @UserId nvarchar(50), @Search nvarchar(200) = NULL, @Page int = 1, @PageSize int = 50
~~~

**api.usp_WMS_LOC02_PutAwayBatches_v1**

~~~sql
api.usp_WMS_LOC02_PutAwayBatches_v1 @UserId nvarchar(50), @LocationCode nvarchar(50), @Batches api.BatchLocationItem_v1 READONLY
~~~
...
| --- | --- | --- |
| 51001 | 403 | Không có quyền đưa batch lên kệ. |
| 51002 | 400 | Vị trí và batch là bắt buộc. |
| 51004 | 404 | Không tìm thấy vị trí. |
| 51009 | 409 | Có batch không tồn tại, đã có vị trí hoặc không còn hoạt động. |
...
| `sp_update_location` | Dữ liệu nghiệp vụ legacy | SP |
...
flowchart TD
    S0["Quét batch"]
    S1["quét vị trí"]
    S2["kiểm tra sức chứa/quy tắc"]
    S3["xác nhận."]
    S0 --> S1
    S1 --> S2
    S2 --> S3
    S3 --> V{"Hợp lệ?"}
    V -- Có --> OK["Hoàn tất LOC-02"]
...
| --- | --- | --- |
| AC-LOC-02-01 | User có quyền mở use case | Màn hình và dữ liệu đúng phạm vi được hiển thị |
| AC-LOC-02-02 | User không có quyền | HTTP 403, không lộ dữ liệu |
| AC-LOC-02-03 | Dữ liệu hợp lệ | Hoàn tất đúng luồng chính |
| AC-LOC-02-04 | Thiếu dữ liệu bắt buộc | HTTP 400/422, chỉ rõ lỗi |
| AC-LOC-02-05 | Đối tượng không tồn tại | HTTP 404, không ghi dở dang |
| AC-LOC-02-06 | Dữ liệu đã thay đổi đồng thời | HTTP 409 hoặc kết quả nhất quán |
| AC-LOC-02-07 | Lỗi hệ thống | HTTP 500 với 'traceId' |
| AC-LOC-02-08 | Kiểm tra audit | User, thời gian và hành động đúng contract |
| AC-LOC-02-09 | Kiểm tra phân quyền dữ liệu | Không thấy dữ liệu ngoài phạm vi |
| AC-LOC-02-10 | Kiểm tra Power Apps dự phòng | Bảng và trạng thái legacy vẫn tương thích |
...
| Nguồn | Tham chiếu |
| --- | --- |
| Hồ sơ nghiệp vụ | 'HO_SO_TONG_THE_UNG_DUNG_MMS.md' – LOC-02 |
...
| SQL source | database/stored-procedures/w4/api.usp_WMS_LOC02_GetPutAwayWorklist_v1.sql, database/stored-procedures/w4/api.usp_WMS_LOC02_PutAwayBatches_v1.sql |
