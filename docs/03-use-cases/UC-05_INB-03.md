# UC-05 (INB-03) — QUÉT MÃ VẠCH KIỂM ĐẾM & IN TEM NHÃN THÙNG / LÔ (BATCH BARCODE)

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-05 (INB-03)` |
| Use Case Name | Quét Mã Vạch Kiểm Đếm & In Tem Nhãn Thùng / Lô (Batch Barcode) |
| Module | `WMS / Inbound Barcode Labeling` |
...
Kiểm đếm thực tế số lượng thùng/kiện giao đến, quy chuẩn đóng gói theo quy cách tiêu chuẩn (Standard Pack Size). Hệ thống tự động sinh Mã Lô định danh duy nhất (id_nhapkho), sinh Barcode Code 128 và gửi lệnh in tem dán lên từng thùng trước khi đưa vào kiểm định QC và cất kệ.

### 1.2. Primary Actor
- **Nhân viên tiếp nhận kho**
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`ReceivingBatchLabelModal.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_WMS_INB03_GenerateBatchLabels_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền và chứng từ ở trạng thái `UNLABELED`.  
**When** Gửi yêu cầu tiếp nhận / cất kệ / nhập kho thành công.  
**Then** Trả 200 OK, CSDL chuyển sang `LABELED (PENDING_QC)`, ghi Sổ Cái Kép và phát âm thanh phản hồi.
...
### 6.1. Frontend — React (`ReceivingBatchLabelModal.tsx`)
...
- **Stored Procedure:** `api.usp_WMS_INB03_GenerateBatchLabels_v1`
...
sequenceDiagram
    autonumber
    actor User as Nhân viên tiếp nhận kho
    participant UI as React UI (ReceivingBatchLabelModal.tsx)
    ...
    API->>DB: 5. EXEC api.usp_WMS_INB03_GenerateBatchLabels_v1 @UserId, @Params
    
    activate DB
    Note over DB: BƯỚC 1: SET XACT_ABORT ON & Kiểm tra quyền
    Note over DB: BƯỚC 2: BEGIN TRANSACTION & Khóa dữ liệu mục tiêu (UPDLOCK)
    Note over DB: BƯỚC 3: Kiểm tra điều kiện nghiệp vụ (UNLABELED)
...
```mermaid
flowchart TD
    User["Nhân viên tiếp nhận kho"]
    ReactUI["React UI (ReceivingBatchLabelModal.tsx)"]
    BackendAPI["Backend API (.NET 8)"]
    AuthCheck{"Token hợp lệ & Có quyền?"}
    StatusCheck{"Trạng thái hợp lệ (UNLABELED)?"}
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_INB03_GenerateBatchLabels_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật LABELED (PENDING_QC)"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `LABELED (PENDING_QC)` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_WMS_INB03_GenerateBatchLabels_v1` chuẩn Transaction.
