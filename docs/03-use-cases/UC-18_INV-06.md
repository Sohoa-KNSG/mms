# UC-18 (INV-06) — TÁCH LÔ (SPLIT BATCH) & QUẢN LÝ THÙNG LẺ TRONG KHO

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-18 (INV-06)` |
| Use Case Name | Tách Lô (Split Batch) & Quản Lý Thùng Lẻ Trong Kho |
| Module | `WMS / Batch Management` |
...
Cho phép tách một Lô hàng nguyên thùng lớn (Lô Mẹ) thành các Lô con nhỏ hơn để xuất lẻ, chuyển kệ hoặc ghi nhận đếm từng thùng. Hệ thống tạo mã Lô con mới kế thừa thuộc tính Lô mẹ, trừ tồn Lô mẹ, bảo toàn tổng sản lượng và in tem Barcode Lô con tức thời.

### 1.2. Primary Actor
- **Thủ kho / Nhân viên đếm hàng**
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`SplitBatchModal.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_WMS_INV06_SplitBatch_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền và dữ liệu ở trạng thái `PARENT_ACTIVE`.  
**When** Gửi yêu cầu thực hiện thao tác.  
**Then** Trả 200 OK, CSDL chuyển sang `SPLIT_SUCCESS (Lô Con Sinh Mới)`, ghi Sổ Cái Kép và phát âm thanh phản hồi.
...
### 6.1. Frontend — React (`SplitBatchModal.tsx`)
...
- **Stored Procedure:** `api.usp_WMS_INV06_SplitBatch_v1`
...
sequenceDiagram
    autonumber
    actor User as Thủ kho / Nhân viên đếm hàng
    ...
    API->>DB: 5. EXEC api.usp_WMS_INV06_SplitBatch_v1 @UserId, @Params
    
    activate DB
    Note over DB: BƯỚC 1: SET XACT_ABORT ON & Kiểm tra quyền
    Note over DB: BƯỚC 2: BEGIN TRANSACTION & Khóa dữ liệu mục tiêu (UPDLOCK)
    Note over DB: BƯỚC 3: Kiểm tra điều kiện nghiệp vụ (PARENT_ACTIVE)
...
flowchart TD
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_INV06_SplitBatch_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật SPLIT_SUCCESS (Lô Con Sinh Mới)"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `SPLIT_SUCCESS (Lô Con Sinh Mới)` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_WMS_INV06_SplitBatch_v1` chuẩn Transaction.
