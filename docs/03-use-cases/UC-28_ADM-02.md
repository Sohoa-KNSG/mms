# UC-28 (ADM-02) — PHÂN QUYỀN VAI TRÒ & MA TRẬN MÀN HÌNH TRUY CẬP (RBAC)

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-28 (ADM-02)` |
| Use Case Name | Phân Quyền Vai Trò & Ma Trận Màn Hình Truy Cập (RBAC) |
| Module | `WMS / Role & Permission Matrix` |
...
Thiết lập ma trận phân quyền dựa trên vai trò (RBAC) và theo từng tài khoản cụ thể. Quản lý quyền truy cập các mã màn hình chức năng qua view api.vw_SEC_UserScreenAccess_v1.
...
| `BR-${doc.id.replace(/[^a-zA-Z0-9]/g, '')}-03` | Status Gate | Dữ liệu mục tiêu phải ở trạng thái hợp lệ (`UNASSIGNED`). | `409 Conflict` |
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`AdminRolePermissionMatrixPage.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_ADM_SavePermissions_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền và dữ liệu ở trạng thái `UNASSIGNED`.  
**When** Gửi yêu cầu thực hiện thao tác thành công.  
**Then** Trả 200 OK, CSDL chuyển sang `GRANTED`, ghi Audit Log và phát âm thanh phản hồi.
...
|---|:---:|:---:|:---:|:---:|---|
| `dbo.tbl_sec_role_screens` | **X** | **X** | **X** | - | Bảng thực thể chính xử lý nghiệp vụ |
...
sequenceDiagram
    autonumber
    actor User as Quản trị viên hệ thống (Admin)
    participant UI as React UI (AdminRolePermissionMatrixPage.tsx)
...
```mermaid
flowchart TD
    User["Quản trị viên hệ thống (Admin)"]
    ReactUI["React UI (AdminRolePermissionMatrixPage.tsx)"]
    BackendAPI["Backend API (.NET 8)"]
    AuthCheck{"Token hợp lệ & Có quyền?"}
    StatusCheck{"Trạng thái hợp lệ (UNASSIGNED)?"}
    ...
    ProcessLock -->|"5. Execute api.usp_ADM_SavePermissions_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật GRANTED"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `GRANTED` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_ADM_SavePermissions_v1` chuẩn Transaction.
