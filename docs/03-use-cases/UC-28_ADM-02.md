---
title: "Đặc tả ADM-02 - Quản trị danh mục và cấu hình"
use_case_id: "ADM-02"
version: "1.0"
date: "2026-08-13"
...
| Màn hình quyền | `Các màn hình admin/QC` |
...
Duy trì dữ liệu nền phục vụ nghiệp vụ.
...
| Thành công | Hoàn tất đúng luồng 'ADM-02', trả dữ liệu/kết quả contract và ghi audit khi có command. |
...
Bản ghi đang được tham chiếu: không xóa cứng.

### 1.5. Business Rules

| Mã | Quy tắc |
| --- | --- |
| BR-ADM-02-01 | User phải có phiên xác thực và quyền màn hình tương ứng. |
| BR-ADM-02-02 | Dữ liệu bắt buộc phải được trim và kiểm tra tại API lẫn stored procedure. |
| BR-ADM-02-03 | Không tin 'UserId', trạng thái hoặc giá trị suy diễn do client tự gửi. |
| BR-ADM-02-04 | Stored procedure là nơi thực thi logic nghiệp vụ và phân quyền dữ liệu. |
| BR-ADM-02-05 | Command phải nguyên tử, rollback khi bất kỳ bước nào lỗi. |
| BR-ADM-02-06 | Giữ nguyên cấu trúc bảng và mã trạng thái legacy. |
| BR-ADM-02-07 | Lỗi nghiệp vụ phải dùng mã ổn định; lỗi ngoài dự kiến phải có 'traceId'. |
| BR-ADM-02-08 | React không truy cập bảng SQL trực tiếp. |
...
- Header hiển thị mã 'ADM-02' và tên “Quản trị danh mục và cấu hình”.
...
| Screen | Route |
| --- | --- |
| Các màn hình admin/QC | Theo route registry |

### 3.2. .NET endpoint contract

| Endpoint name | Vai trò |
| --- | --- |
| `ADM-02_GetConfigurationCatalog` | .NET endpoint đã định danh |
| `ADM-02_SaveConfiguration` | .NET endpoint đã định danh |
...
| --- | --- |
| `api.usp_WMS_ADM02_GetConfigurationCatalog_v1` | `database/stored-procedures/w2/api.usp_WMS_ADM02_GetConfigurationCatalog_v1.sql` |
| `api.usp_WMS_ADM02_SaveConfiguration_v1` | `database/stored-procedures/w2/api.usp_WMS_ADM02_SaveConfiguration_v1.sql` |

**api.usp_WMS_ADM02_GetConfigurationCatalog_v1**

~~~sql
api.usp_WMS_ADM02_GetConfigurationCatalog_v1 @UserId nvarchar(50), @CatalogCode nvarchar(40)
~~~

**api.usp_WMS_ADM02_SaveConfiguration_v1**

~~~sql
api.usp_WMS_ADM02_SaveConfiguration_v1 @UserId nvarchar(50), @CatalogCode nvarchar(40), @KeyCode nvarchar(50), @Name nvarchar(100), @Description nvarchar(255) = NULL, @LogicValue nvarchar(50) = NULL, @DisplayValue nvarchar(100) = NULL, @ExpectedChangedAt datetime2(7) = NULL
~~~
...
| --- | --- | --- |
| 51001 | 403 | Không có quyền quản trị danh mục. |
| 51022 | 422 | Danh mục không nằm trong whitelist quản trị. |
| 51022 | 422 | Mã và tên danh mục là bắt buộc. |
| 51009 | 409 | Danh mục đã được cập nhật. Hãy tải lại dữ liệu. |
| 51022 | 422 | Logic nghiệp vụ kho chỉ nhận -1, 0 hoặc 1. |
| 51022 | 422 | Danh mục trạng thái tồn legacy không có thời điểm sửa để kiểm tra concurrency. |
| 51022 | 422 | Logic tồn chỉ nhận -1, 0 hoặc 1. |
| 51022 | 422 | Danh mục sự kiện batch legacy không có thời điểm sửa để kiểm tra concurrency. |
...
| --- | --- | --- |
| `tbl_dm_*, tbl_khaibao_qc, tbl_nhom_vattu_qc` | Dữ liệu nghiệp vụ legacy | SP |
...
sequenceDiagram
    autonumber
    actor User
    participant UI as React ADM-02
    participant API as .NET API
    participant SP as SQL Contract
    participant DB as MMS Legacy Tables
    User->>UI: Thực hiện Quản trị danh mục và cấu hình
...
flowchart TD
    S0["Chọn danh mục"]
    S1["thêm/sửa"]
    S2["kiểm tra tham chiếu"]
    S3["lưu."]
    S0 --> S1
    S1 --> S2
    S2 --> S3
    S3 --> V{"Hợp lệ?"}
    V -- Có --> OK["Hoàn tất ADM-02"]
...
| --- | --- | --- |
| AC-ADM-02-01 | User có quyền mở use case | Màn hình và dữ liệu đúng phạm vi được hiển thị |
| AC-ADM-02-02 | User không có quyền | HTTP 403, không lộ dữ liệu |
| AC-ADM-02-03 | Dữ liệu hợp lệ | Hoàn tất đúng luồng chính |
| AC-ADM-02-04 | Thiếu dữ liệu bắt buộc | HTTP 400/422, chỉ rõ lỗi |
| AC-ADM-02-05 | Đối tượng không tồn tại | HTTP 404, không ghi dở dang |
| AC-ADM-02-06 | Dữ liệu đã thay đổi đồng thời | HTTP 409 hoặc kết quả nhất quán |
| AC-ADM-02-07 | Lỗi hệ thống | HTTP 500 với 'traceId' |
| AC-ADM-02-08 | Kiểm tra audit | User, thời gian và hành động đúng contract |
| AC-ADM-02-09 | Kiểm tra phân quyền dữ liệu | Không thấy dữ liệu ngoài phạm vi |
| AC-ADM-02-10 | Kiểm tra Power Apps dự phòng | Bảng và trạng thái legacy vẫn tương thích |
...
| Nguồn | Tham chiếu |
| --- | --- |
| Hồ sơ nghiệp vụ | 'HO_SO_TONG_THE_UNG_DUNG_MMS.md' – ADM-02 |
| Route registry | 'apps/web/src/app/routeRegistry.ts' |
| API source | apps/api/Modules/Administration/AdministrationEndpoints.cs |
| SQL source | database/stored-procedures/w2/api.usp_WMS_ADM02_GetConfigurationCatalog_v1.sql, database/stored-procedures/w2/api.usp_WMS_ADM02_SaveConfiguration_v1.sql |
