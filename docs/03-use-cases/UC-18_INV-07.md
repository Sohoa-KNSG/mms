# UC-18 (INV-07) — SƠ ĐỒ CÂY GIA PHẢ LÔ (GENEALOGY TREE LÔ MẸ - LÔ CON)

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-18 (INV-07)` |
| Use Case Name | Sơ Đồ Cây Gia Phả Lô (Genealogy Tree Lô Mẹ - Lô Con) |
| Module | `WMS / Batch Genealogy` |
...
Trực quan hóa toàn bộ phả hệ nguồn gốc và quá trình phân rã của Lô hàng từ Lô nhập kho ban đầu của Nhà cung cấp (Gốc F0) qua các lần tách thùng lẻ (F1, F2...) đến các lần xuất kho vào phân xưởng. Hỗ trợ truy vết 2 chiều thuận - nghịch phục vụ kiểm toán chất lượng.

### 1.2. Primary Actor
- **Thủ kho / QC / Ban Giám Đốc**
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`BatchGenealogyTree.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_WMS_INV07_GetBatchGenealogy_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền và dữ liệu ở trạng thái `TRACED`.  
**When** Gửi yêu cầu thực hiện thao tác.  
**Then** Trả 200 OK, CSDL chuyển sang `VISUALIZED`, ghi Sổ Cái Kép và phát âm thanh phản hồi.
...
### 6.1. Frontend — React (`BatchGenealogyTree.tsx`)
...
- **Stored Procedure:** `api.usp_WMS_INV07_GetBatchGenealogy_v1`
...
sequenceDiagram
    autonumber
    actor User as Thủ kho / QC / Ban Giám Đốc
    participant UI as React UI (BatchGenealogyTree.tsx)
    ...
    API->>DB: 5. EXEC api.usp_WMS_INV07_GetBatchGenealogy_v1 @UserId, @Params
    
    activate DB
    Note over DB: BƯỚC 1: SET XACT_ABORT ON & Kiểm tra quyền
    Note over DB: BƯỚC 2: BEGIN TRANSACTION & Khóa dữ liệu mục tiêu (UPDLOCK)
    Note over DB: BƯỚC 3: Kiểm tra điều kiện nghiệp vụ (TRACED)
...
```mermaid
flowchart TD
    User["Thủ kho / QC / Ban Giám Đốc"]
    ReactUI["React UI (BatchGenealogyTree.tsx)"]
    BackendAPI["Backend API (.NET 8)"]
    AuthCheck{"Token hợp lệ & Có quyền?"}
    StatusCheck{"Trạng thái hợp lệ (TRACED)?"}
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_INV07_GetBatchGenealogy_v1"| DB
...
- [x] API Contract & Stored Procedure `api.usp_WMS_INV07_GetBatchGenealogy_v1` chuẩn Transaction.
