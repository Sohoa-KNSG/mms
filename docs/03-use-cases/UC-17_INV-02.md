# UC-17 (INV-02) — TRA CỨU TỒN KHO CHI TIẾT THEO LÔ (BATCH) & HẠN SỬ DỤNG

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-17 (INV-02)` |
| Use Case Name | Tra Cứu Tồn Kho Chi Tiết Theo Lô (Batch) & Hạn Sử Dụng |
| Module | `WMS / Inventory Batch Inquiry` |
...
Quản lý và truy vết toàn diện 11,665 Lô hàng tồn kho. Cung cấp thông tin định danh duy nhất của từng Lô: Mã Lô (id_nhapkho), Mã SKU, Số lượng tồn, Vị trí Ô kệ, Trạng thái QC (PASS/REJECT/PENDING), Hạn sử dụng (Exp Date) và thông tin Nhà Cung Cấp.
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`InventoryBatchTab.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_WMS_INV02_GetStockByBatch_v1`.
...
**Then** Trả 200 OK, CSDL chuyển sang `ACTIVE`, ghi Sổ Cái Kép và phát âm thanh phản hồi.
...
### 6.1. Frontend — React (`InventoryBatchTab.tsx`)
...
- **Stored Procedure:** `api.usp_WMS_INV02_GetStockByBatch_v1`
...
sequenceDiagram
    autonumber
    actor User as Thủ kho / KCS / QC
    participant UI as React UI (InventoryBatchTab.tsx)
    ...
    API->>DB: 5. EXEC api.usp_WMS_INV02_GetStockByBatch_v1 @UserId, @Params
...
flowchart TD
    User["Thủ kho / KCS / QC"]
    ReactUI["React UI (InventoryBatchTab.tsx)"]
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_INV02_GetStockByBatch_v1"| DB
...
- [x] API Contract & Stored Procedure `api.usp_WMS_INV02_GetStockByBatch_v1` chuẩn Transaction.
