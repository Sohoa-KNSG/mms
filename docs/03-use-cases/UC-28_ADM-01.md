# UC-28 (ADM-01) — QUẢN LÝ DANH MỤC NGƯỜI DÙNG & TÀI KHOẢN NHÂN VIÊN

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-28 (ADM-01)` |
| Use Case Name | Quản Lý Danh Mục Người Dùng & Tài Khoản Nhân Viên |
| Module | `WMS / User Administration` |
...
Quản lý toàn bộ danh sách tài khoản người dùng (tbl_dm_user), liên kết mã nhân viên, họ tên, phòng ban phân xưởng, email, trạng thái kích hoạt (status_active), cấp phát và đặt lại mật khẩu cho nhân viên kho và phân xưởng.
...
| `BR-${doc.id.replace(/[^a-zA-Z0-9]/g, '')}-03` | Status Gate | Dữ liệu mục tiêu phải ở trạng thái hợp lệ (`INACTIVE`). | `409 Conflict` |
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`AdminUserManagementPage.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_ADM_ManageUsers_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền và dữ liệu ở trạng thái `INACTIVE`.  
**When** Gửi yêu cầu thực hiện thao tác thành công.  
**Then** Trả 200 OK, CSDL chuyển sang `ACTIVE`, ghi Audit Log và phát âm thanh phản hồi.
...
sequenceDiagram
    ...
    participant UI as React UI (AdminUserManagementPage.tsx)
...
flowchart TD
    ...
    ReactUI["React UI (AdminUserManagementPage.tsx)"]
    BackendAPI["Backend API (.NET 8)"]
    AuthCheck{"Token hợp lệ & Có quyền?"}
    StatusCheck{"Trạng thái hợp lệ (INACTIVE)?"}
    ...
    ProcessLock -->|"5. Execute api.usp_ADM_ManageUsers_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật ACTIVE"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `ACTIVE` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_ADM_ManageUsers_v1` chuẩn Transaction.
