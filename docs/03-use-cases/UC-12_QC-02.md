# UC-12 (QC-02) — THIẾT LẬP TIÊU CHÍ KIỂM TRA KỸ THUẬT & PHƯƠNG PHÁP LẤY MẪU (AQL / 100%)

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-12 (QC-02)` |
| Use Case Name | Thiết Lập Tiêu Chí Kiểm Tra Kỹ Thuật & Phương Pháp Lấy Mẫu (AQL / 100%) |
| Module | `WMS / QC Criteria` |
...
Định nghĩa bộ tiêu chí kỹ thuật kiểm tra cho từng nhóm SKU (kích thước, độ dày, độ cứng HRC, độ bóng...) và tự động áp dụng bảng tiêu chuẩn lấy mẫu thống kê AQL hoặc kiểm tra 100% toàn bộ số lượng Lô.
...
| `BR-${doc.id.replace(/[^a-zA-Z0-9]/g, '')}-03` | Status Gate | Dữ liệu mục tiêu phải ở trạng thái hợp lệ (`CONFIG`). | `409 Conflict` |
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`QcCriteriaConfigModal.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_WMS_QC02_GetInspectionCriteria_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền và dữ liệu ở trạng thái `CONFIG`.  
**When** Gửi yêu cầu thực hiện thao tác thành công.  
**Then** Trả 200 OK, CSDL chuyển sang `CRITERIA_APPLIED`, ghi Audit Log và phát âm thanh phản hồi.
...
sequenceDiagram
    autonumber
    actor User as Trưởng nhóm KCS / QC
    participant UI as React UI (QcCriteriaConfigModal.tsx)
    ...
    API->>DB: 5. EXEC api.usp_WMS_QC02_GetInspectionCriteria_v1 @UserId, @Params
    
    activate DB
    Note over DB: BƯỚC 1: SET XACT_ABORT ON & Kiểm tra quyền
    Note over DB: BƯỚC 2: BEGIN TRANSACTION & Khóa dữ liệu mục tiêu (UPDLOCK)
    Note over DB: BƯỚC 3: Kiểm tra điều kiện nghiệp vụ (CONFIG)
...
```mermaid
flowchart TD
    User["Trưởng nhóm KCS / QC"]
    ReactUI["React UI (QcCriteriaConfigModal.tsx)"]
    BackendAPI["Backend API (.NET 8)"]
    AuthCheck{"Token hợp lệ & Có quyền?"}
    StatusCheck{"Trạng thái hợp lệ (CONFIG)?"}
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_QC02_GetInspectionCriteria_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật CRITERIA_APPLIED"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `CRITERIA_APPLIED` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_WMS_QC02_GetInspectionCriteria_v1` chuẩn Transaction.
