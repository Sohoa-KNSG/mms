# UC-26 (QC-06) — QUẢN LÝ KHO CÁCH LY & THỦ TỤC TRẢ HÀNG NHÀ CUNG CẤP KHÔNG ĐẠT

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-26 (QC-06)` |
| Use Case Name | Quản Lý Kho Cách Ly & Thủ Tục Trả Hàng Nhà Cung Cấp Không Đạt |
| Module | `WMS / Quarantine Management` |
...
Quản lý toàn bộ các Lô hàng REJECT lưu trữ tại Khu vực kho cách ly (Khu Q), lập biên bản trả hàng cho Nhà cung cấp kèm hình ảnh lỗi và hạch toán giảm trừ công nợ trên ERP Bravo.
...
| `BR-${doc.id.replace(/[^a-zA-Z0-9]/g, '')}-03` | Status Gate | Dữ liệu mục tiêu phải ở trạng thái hợp lệ (`REJECT`). | `409 Conflict` |
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`QuarantineManagementPage.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_WMS_QC06_CreateVendorReturn_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền và dữ liệu ở trạng thái `REJECT`.  
**When** Gửi yêu cầu thực hiện thao tác thành công.  
**Then** Trả 200 OK, CSDL chuyển sang `RETURNED_TO_VENDOR`, ghi Audit Log và phát âm thanh phản hồi.
...
|---|:---:|:---:|:---:|:---:|---|
| `dbo.tbl_map_nhapkho` | **X** | **X** | **X** | - | Bảng thực thể chính xử lý nghiệp vụ |
...
sequenceDiagram
    autonumber
    actor User as QC / Thủ kho
    participant UI as React UI (QuarantineManagementPage.tsx)
    ...
    API->>DB: 5. EXEC api.usp_WMS_QC06_CreateVendorReturn_v1 @UserId, @Params
    
    activate DB
    Note over DB: BƯỚC 1: SET XACT_ABORT ON & Kiểm tra quyền
    Note over DB: BƯỚC 2: BEGIN TRANSACTION & Khóa dữ liệu mục tiêu (UPDLOCK)
    Note over DB: BƯỚC 3: Kiểm tra điều kiện nghiệp vụ (REJECT)
...
flowchart TD
    User["QC / Thủ kho"]
    ReactUI["React UI (QuarantineManagementPage.tsx)"]
    BackendAPI["Backend API (.NET 8)"]
    AuthCheck{"Token hợp lệ & Có quyền?"}
    StatusCheck{"Trạng thái hợp lệ (REJECT)?"}
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_QC06_CreateVendorReturn_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật RETURNED_TO_VENDOR"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `RETURNED_TO_VENDOR` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_WMS_QC06_CreateVendorReturn_v1` chuẩn Transaction.
