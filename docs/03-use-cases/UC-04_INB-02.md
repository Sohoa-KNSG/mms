# UC-04 (INB-02) — TIẾP NHẬN VẬT TƯ KHÔNG THEO PO (ĐỘT XUẤT / PHI PO)

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-04 (INB-02)` |
| Use Case Name | Tiếp Nhận Vật Tư Không Theo PO (Đột Xuất / Phi PO) |
| Module | `WMS / Non-PO Receiving` |
...
Cho phép nhập kho các loại hàng mẫu, vật tư tặng kèm, hàng tài trợ, phế liệu thu hồi hoặc vật tư phát sinh ngoài quy trình PO Bravo thông thường. Bắt buộc khai báo nguồn gốc và mở phiên tiếp nhận đặc biệt (INB_NON_PO).
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`ReceivingNonPoPage.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_WMS_INB02_CreateNonPoReceipt_v1`.
...
### 6.1. Frontend — React (`ReceivingNonPoPage.tsx`)
...
- **Stored Procedure:** `api.usp_WMS_INB02_CreateNonPoReceipt_v1`
...
sequenceDiagram
    ...
    participant UI as React UI (ReceivingNonPoPage.tsx)
    ...
    API->>DB: 5. EXEC api.usp_WMS_INB02_CreateNonPoReceipt_v1 @UserId, @Params
...
flowchart TD
    User["Thủ kho / KCS / QC"]
    ReactUI["React UI (ReceivingNonPoPage.tsx)"]
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_INB02_CreateNonPoReceipt_v1"| DB
...
- [x] API Contract & Stored Procedure `api.usp_WMS_INB02_CreateNonPoReceipt_v1` chuẩn Transaction.
