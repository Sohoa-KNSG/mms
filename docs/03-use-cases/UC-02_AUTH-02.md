# UC-02 (AUTH-02) — ĐĂNG XUẤT & THU HỒI PHIÊN LÀM VIỆC AN TOÀN (LOGOUT & REVOCATION)

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-02 (AUTH-02)` |
| Use Case Name | Đăng Xuất & Thu Hồi Phiên Làm Việc An Toàn (Logout & Revocation) |
...
Hủy bỏ phiên làm việc của người dùng, xóa bỏ HttpOnly Cookie phía client, đưa token vào danh sách thu hồi (Revoked Tokens Cache), ghi nhận thời điểm đăng xuất vào nhật ký bảo mật và chuyển hướng về màn hình đăng nhập.
...
| `BR-${doc.id.replace(/[^a-zA-Z0-9]/g, '')}-03` | Status Gate | Dữ liệu mục tiêu phải ở trạng thái hợp lệ (`AUTHENTICATED`). | `409 Conflict` |
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`UserNavbarDropdown.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_SEC_Logout_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền và dữ liệu ở trạng thái `AUTHENTICATED`.  
**When** Gửi yêu cầu thực hiện thao tác thành công.  
**Then** Trả 200 OK, CSDL chuyển sang `LOGGED_OUT`, ghi Audit Log và phát âm thanh phản hồi.
...
sequenceDiagram
    ...
    participant UI as React UI (UserNavbarDropdown.tsx)
...
flowchart TD
    ...
    ReactUI["React UI (UserNavbarDropdown.tsx)"]
    BackendAPI["Backend API (.NET 8)"]
    AuthCheck{"Token hợp lệ & Có quyền?"}
    StatusCheck{"Trạng thái hợp lệ (AUTHENTICATED)?"}
    ...
    ProcessLock -->|"5. Execute api.usp_SEC_Logout_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật LOGGED_OUT"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `LOGGED_OUT` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_SEC_Logout_v1` chuẩn Transaction.
