# UC-27 (INV-08) — LẬP KẾ HOẠCH KIỂM KÊ TỒN KHO ĐỊNH KỲ & ĐẾM MÙ

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-27 (INV-08)` |
| Use Case Name | Lập Kế Hoạch Kiểm Kê Tồn Kho Định Kỳ & Đếm Mù |
| Module | `WMS / Cycle Count Planning` |
...
Cho phép thiết lập các đợt kiểm kê tồn kho định kỳ hoặc đột xuất theo Nhóm SKU, Khu vực Ô kệ. Hệ thống chốt số liệu tồn sổ sách tại thời điểm lập kế hoạch (Freeze Snapshot) và kích hoạt cơ chế Đếm Mù (Blind Count) tuyệt đối không hiển thị số sổ sách trên PDA.

### 1.2. Primary Actor
- **Trưởng phòng kho / Ban Giám Đốc**
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`CycleCountPlanPage.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_WMS_INV08_CreateCycleCountPlan_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền và dữ liệu ở trạng thái `DRAFT`.  
**When** Gửi yêu cầu thực hiện thao tác.  
**Then** Trả 200 OK, CSDL chuyển sang `IN_PROGRESS`, ghi Sổ Cái Kép và phát âm thanh phản hồi.
...
### 6.1. Frontend — React (`CycleCountPlanPage.tsx`)
...
- **Stored Procedure:** `api.usp_WMS_INV08_CreateCycleCountPlan_v1`
...
|---|:---:|:---:|:---:|:---:|---|
| `dbo.tbl_kiemke_header` | **X** | **X** | **X** | - | Bảng thực thể chính xử lý tồn kho / kiểm kê |
...
sequenceDiagram
    autonumber
    actor User as Trưởng phòng kho / Ban Giám Đốc
    participant UI as React UI (CycleCountPlanPage.tsx)
    ...
    API->>DB: 5. EXEC api.usp_WMS_INV08_CreateCycleCountPlan_v1 @UserId, @Params
    
    activate DB
    Note over DB: BƯỚC 1: SET XACT_ABORT ON & Kiểm tra quyền
    Note over DB: BƯỚC 2: BEGIN TRANSACTION & Khóa dữ liệu mục tiêu (UPDLOCK)
    Note over DB: BƯỚC 3: Kiểm tra điều kiện nghiệp vụ (DRAFT)
...
```mermaid
flowchart TD
    User["Trưởng phòng kho / Ban Giám Đốc"]
    ReactUI["React UI (CycleCountPlanPage.tsx)"]
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_INV08_CreateCycleCountPlan_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật IN_PROGRESS"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `IN_PROGRESS` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_WMS_INV08_CreateCycleCountPlan_v1` chuẩn Transaction.
