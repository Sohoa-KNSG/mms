# UC-27 (INV-09) — KIỂM ĐẾM THỰC ĐỊA TRÊN PDA, ĐẾM TỪNG THÙNG & CHỐT CHÊNH LỆCH

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-27 (INV-09)` |
| Use Case Name | Kiểm Đếm Thực Địa Trên PDA, Đếm Từng Thùng & Chốt Chênh Lệch |
| Module | `WMS / Cycle Count Execution` |
...
Cung cấp ứng dụng kiểm đếm trên PDA theo quy trình 3 bước công thái học: Bước 1 (Quét Kệ) -> Bước 2 (Quét Lô) -> Bước 3 (Nhập số đếm thùng). Tự động tách thùng lẻ và bật popup in tem mới. Hỗ trợ Trưởng phòng đối soát báo cáo chênh lệch và chốt điều chỉnh tồn kho (ADJUST_COUNT) vào Sổ Cái Kép.

### 1.2. Primary Actor
- **Nhân viên kiểm kê PDA / Trưởng phòng kho**
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`PdaScanner.jsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_WMS_INV09_RecordCount_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền và dữ liệu ở trạng thái `IN_PROGRESS`.  
**When** Gửi yêu cầu thực hiện thao tác.  
**Then** Trả 200 OK, CSDL chuyển sang `RECONCILED (Chốt Sổ Cái)`, ghi Sổ Cái Kép và phát âm thanh phản hồi.
...
### 6.1. Frontend — React (`PdaScanner.jsx`)
...
- **Stored Procedure:** `api.usp_WMS_INV09_RecordCount_v1`
...
|---|:---:|:---:|:---:|:---:|---|
| `dbo.tbl_kiemke_detail` | **X** | **X** | **X** | - | Bảng thực thể chính xử lý tồn kho / kiểm kê |
...
sequenceDiagram
    autonumber
    actor User as Nhân viên kiểm kê PDA / Trưởng phòng kho
    participant UI as React UI (PdaScanner.jsx)
    ...
    API->>DB: 5. EXEC api.usp_WMS_INV09_RecordCount_v1 @UserId, @Params
    
    activate DB
    Note over DB: BƯỚC 1: SET XACT_ABORT ON & Kiểm tra quyền
    Note over DB: BƯỚC 2: BEGIN TRANSACTION & Khóa dữ liệu mục tiêu (UPDLOCK)
    Note over DB: BƯỚC 3: Kiểm tra điều kiện nghiệp vụ (IN_PROGRESS)
...
```mermaid
flowchart TD
    User["Nhân viên kiểm kê PDA / Trưởng phòng kho"]
    ReactUI["React UI (PdaScanner.jsx)"]
    BackendAPI["Backend API (.NET 8)"]
    AuthCheck{"Token hợp lệ & Có quyền?"}
    StatusCheck{"Trạng thái hợp lệ (IN_PROGRESS)?"}
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_INV09_RecordCount_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật RECONCILED (Chốt Sổ Cái)"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `RECONCILED (Chốt Sổ Cái)` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_WMS_INV09_RecordCount_v1` chuẩn Transaction.
