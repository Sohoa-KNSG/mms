# UC-20 (OUT-03) — ĐĂNG KÝ ĐỀ NGHỊ XUẤT KHO VƯỢT ĐỊNH MỨC

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-20 (OUT-03)` |
| Use Case Name | Đăng Ký Đề Nghị Xuất Kho Vượt Định Mức |
...
Cho phép lập đề nghị xuất bù vật tư khi phát sinh hư hỏng, phế phẩm trong quá trình sản xuất vượt quá tỷ lệ hao hụt định mức cho phép. Yêu cầu đính kèm biên bản sự cố và chuyển luồng phê duyệt đặc biệt lên Ban Giám Đốc.
...
### 3.1. Main Flow
1. Người dùng mở màn hình chức năng trên giao diện (`OutboundOverLimitRequestPage.tsx`).
...
6. Backend kiểm tra Middleware Auth, kiểm tra Business Rules và gọi Stored Procedure `api.usp_WMS_OUT03_CreateOverLimitIssueRequest_v1`.
...
**Then** Hệ thống trả về `200 OK`, CSDL cập nhật sang `PENDING_DIRECTOR_APPROVAL (trang_thai_phieu = 1)`, ghi nhật ký Audit Log và phát âm thanh phản hồi.
...
- Component: `OutboundOverLimitRequestPage.tsx`
...
### 6.1. Frontend — React (`OutboundOverLimitRequestPage.tsx`)
...
- Thin API Endpoint nhận DTO, trích xuất Claim và ủy thác cho `api.usp_WMS_OUT03_CreateOverLimitIssueRequest_v1`.

### 6.3. API Contract & Stored Procedure
- **Endpoint:** `POST /api/v1/...`
- **Stored Procedure:** `api.usp_WMS_OUT03_CreateOverLimitIssueRequest_v1`
...
|---|---|---|---|---|
| `dbo.tbl_phieu_yeucau` | `trang_thai` | `DRAFT` | `PENDING_DIRECTOR_APPROVAL (trang_thai_phieu = 1)` | Chuyển đổi trạng thái nghiệp vụ thành công |
...
sequenceDiagram
    autonumber
    actor User as Quản đốc phân xưởng
    participant UI as React UI (OutboundOverLimitRequestPage.tsx)
    ...
    API->>DB: 5. EXEC api.usp_WMS_OUT03_CreateOverLimitIssueRequest_v1 @UserId, @Params
...
```mermaid
flowchart TD
    User["Quản đốc phân xưởng"]
    ReactUI["React UI (OutboundOverLimitRequestPage.tsx)"]
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_OUT03_CreateOverLimitIssueRequest_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật PENDING_DIRECTOR_APPROVAL (trang_thai_phieu = 1)"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `PENDING_DIRECTOR_APPROVAL (trang_thai_phieu = 1)` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_WMS_OUT03_CreateOverLimitIssueRequest_v1` chuẩn Transaction.
