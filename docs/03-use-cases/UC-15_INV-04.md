# UC-15 (INV-04) — CẢNH BÁO MỨC TỒN AN TOÀN MIN/MAX & HẠN SỬ DỤNG LÔ

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-15 (INV-04)` |
| Use Case Name | Cảnh Báo Mức Tồn An Toàn Min/Max & Hạn Sử Dụng Lô |
| Module | `WMS / Inventory Warning` |
...
Tự động phát cảnh báo các SKU có tồn khả dụng dưới mức Min (Nguy cơ đứt hàng sản xuất), vượt mức Max (Ùn ứ kho) hoặc các Lô hàng cận hạn sử dụng (< 30 ngày) và hàng tồn đọng lâu ngày (> 180 ngày).

### 1.2. Primary Actor
- **Thủ kho / Phòng Thu Mua / Kế Hoạch**
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`InventorySafetyWarningPage.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_WMS_INV04_GetStockAlerts_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền và dữ liệu ở trạng thái `WARNING`.  
**When** Gửi yêu cầu thực hiện thao tác.  
**Then** Trả 200 OK, CSDL chuyển sang `ALERTED`, ghi Sổ Cái Kép và phát âm thanh phản hồi.
...
### 6.1. Frontend — React (`InventorySafetyWarningPage.tsx`)
...
- **Stored Procedure:** `api.usp_WMS_INV04_GetStockAlerts_v1`
...
|---|:---:|:---:|:---:|:---:|---|
| `dbo.tbl_dm_vattu` | **X** | **X** | **X** | - | Bảng thực thể chính xử lý tồn kho / kiểm kê |
...
sequenceDiagram
    autonumber
    actor User as Thủ kho / Phòng Thu Mua / Kế Hoạch
    participant UI as React UI (InventorySafetyWarningPage.tsx)
    ...
    API->>DB: 5. EXEC api.usp_WMS_INV04_GetStockAlerts_v1 @UserId, @Params
    
    activate DB
    Note over DB: BƯỚC 1: SET XACT_ABORT ON & Kiểm tra quyền
    Note over DB: BƯỚC 2: BEGIN TRANSACTION & Khóa dữ liệu mục tiêu (UPDLOCK)
    Note over DB: BƯỚC 3: Kiểm tra điều kiện nghiệp vụ (WARNING)
...
```mermaid
flowchart TD
    User["Thủ kho / Phòng Thu Mua / Kế Hoạch"]
    ReactUI["React UI (InventorySafetyWarningPage.tsx)"]
    BackendAPI["Backend API (.NET 8)"]
    AuthCheck{"Token hợp lệ & Có quyền?"}
    StatusCheck{"Trạng thái hợp lệ (WARNING)?"}
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_INV04_GetStockAlerts_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật ALERTED"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `ALERTED` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_WMS_INV04_GetStockAlerts_v1` chuẩn Transaction.
