# UC-10 (INB-08) — NHẬP KHO BÁN THÀNH PHẨM & THÀNH PHẨM TỪ PHÂN XƯỞNG SẢN XUẤT

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-10 (INB-08)` |
| Use Case Name | Nhập Kho Bán Thành Phẩm & Thành Phẩm Từ Phân Xưởng Sản Xuất |
| Module | `WMS / Production Inbound` |
...
Tiếp nhận các Lô bán thành phẩm (BTP) hoặc thành phẩm (TP) sau khi hoàn tất công đoạn dập/mài/xi mạ từ các phân xưởng sản xuất bàn giao về kho lưu trữ, gắn mã Lô BTP và đưa vào vị trí Ô kệ quy định.

### 1.2. Primary Actor
- **Đại diện phân xưởng / Thủ kho**
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`ProductionReceiptPage.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_WMS_INB08_CreateProductionReceipt_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền và chứng từ ở trạng thái `PRODUCED`.  
**When** Gửi yêu cầu tiếp nhận / cất kệ / nhập kho thành công.  
**Then** Trả 200 OK, CSDL chuyển sang `STORED`, ghi Sổ Cái Kép và phát âm thanh phản hồi.
...
### 6.1. Frontend — React (`ProductionReceiptPage.tsx`)
...
- **Stored Procedure:** `api.usp_WMS_INB08_CreateProductionReceipt_v1`
...
sequenceDiagram
    autonumber
    actor User as Đại diện phân xưởng / Thủ kho
    participant UI as React UI (ProductionReceiptPage.tsx)
    ...
    API->>DB: 5. EXEC api.usp_WMS_INB08_CreateProductionReceipt_v1 @UserId, @Params
    
    activate DB
    Note over DB: BƯỚC 1: SET XACT_ABORT ON & Kiểm tra quyền
    Note over DB: BƯỚC 2: BEGIN TRANSACTION & Khóa dữ liệu mục tiêu (UPDLOCK)
    Note over DB: BƯỚC 3: Kiểm tra điều kiện nghiệp vụ (PRODUCED)
...
```mermaid
flowchart TD
    User["Đại diện phân xưởng / Thủ kho"]
    ReactUI["React UI (ProductionReceiptPage.tsx)"]
    BackendAPI["Backend API (.NET 8)"]
    AuthCheck{"Token hợp lệ & Có quyền?"}
    StatusCheck{"Trạng thái hợp lệ (PRODUCED)?"}
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_INB08_CreateProductionReceipt_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật STORED"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `STORED` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_WMS_INB08_CreateProductionReceipt_v1` chuẩn Transaction.
