# UC-12 (QC-01) — TIẾP NHẬN DANH SÁCH LÔ HÀNG CHỜ KIỂM ĐỊNH CHẤT LƯỢNG (QC QUEUE)

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-12 (QC-01)` |
| Use Case Name | Tiếp Nhận Danh Sách Lô Hàng Chờ Kiểm Định Chất Lượng (QC Queue) |
| Module | `WMS / QC Queue` |
...
Sau khi hàng được tiếp nhận và in tem nhãn tại cửa kho, các Lô tự động được đưa vào hàng đợi kiểm tra chất lượng (status_qc = PENDING). Phân hệ QC hiển thị danh sách Lô chờ kiểm, phân loại độ ưu tiên và chỉ định KCS lấy mẫu.
...
| `BR-${doc.id.replace(/[^a-zA-Z0-9]/g, '')}-03` | Status Gate | Dữ liệu mục tiêu phải ở trạng thái hợp lệ (`PENDING`). | `409 Conflict` |
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`QcInspectionQueuePage.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_WMS_QC01_GetInspectionQueue_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền và dữ liệu ở trạng thái `PENDING`.  
**When** Gửi yêu cầu thực hiện thao tác thành công.  
**Then** Trả 200 OK, CSDL chuyển sang `IN_INSPECTION`, ghi Audit Log và phát âm thanh phản hồi.
...
sequenceDiagram
    ...
    participant UI as React UI (QcInspectionQueuePage.tsx)
    ...
    API->>DB: 5. EXEC api.usp_WMS_QC01_GetInspectionQueue_v1 @UserId, @Params
    
    activate DB
    Note over DB: BƯỚC 1: SET XACT_ABORT ON & Kiểm tra quyền
    Note over DB: BƯỚC 2: BEGIN TRANSACTION & Khóa dữ liệu mục tiêu (UPDLOCK)
    Note over DB: BƯỚC 3: Kiểm tra điều kiện nghiệp vụ (PENDING)
...
flowchart TD
    ...
    ReactUI["React UI (QcInspectionQueuePage.tsx)"]
    BackendAPI["Backend API (.NET 8)"]
    AuthCheck{"Token hợp lệ & Có quyền?"}
    StatusCheck{"Trạng thái hợp lệ (PENDING)?"}
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_QC01_GetInspectionQueue_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật IN_INSPECTION"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `IN_INSPECTION` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_WMS_QC01_GetInspectionQueue_v1` chuẩn Transaction.
