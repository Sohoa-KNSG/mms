...
| :--- | :---: | :---: | :---: | :---: | :--- |
| `dbo.tbl_qc_inspection` | **X** | **X** | **X** | - | Tạo hồ sơ KCS, Cập nhật kết luận kiểm định `decision = 'PASS'/'REJECT'` |
| `dbo.tbl_qc_measurements` | **X** | **X** | - | - | Ghi nhận kết quả đo lường kích thước, độ cứng, dung sai của các mẫu thử |
| `dbo.tbl_map_nhapkho` | - | **X** | **X** | - | Cập nhật trạng thái chất lượng Lô (`status_qc = 'PASS'` hoặc `'REJECT'`) |
| `dbo.audit_log` | **X** | **X** | - | - | Ghi vết biên bản kiểm tra KCS và chữ ký số người duyệt |

### 4.2. Định nghĩa Trạng thái (Conceptual State Model)

| Cột / Biến | Kiểu Dữ Liệu | Giá Trị Sau Confirm | Ý nghĩa Nghiệp vụ |
| :--- | :--- | :--- | :--- |
| `decision` (trong `tbl_qc_inspection`) | `NVARCHAR(20)` | `'PASS'` / `'REJECT'` | Kết luận kiểm định chính thức của Trưởng nhóm KCS |
| `status_qc` (trong `tbl_map_nhapkho`) | `VARCHAR(20)` | `'PASS'` / `'REJECT'` | Trạng thái chất lượng giải phóng Lô hàng cất kệ hoặc đưa vào kho cách ly |
| `status_kho` (trong `tbl_map_nhapkho`) | `VARCHAR(20)` | `'ON_RACK'` / `'QUARANTINE'` | Vị trí lưu kho sau quyết định kiểm định |
