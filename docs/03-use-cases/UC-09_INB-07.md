# UC-09 (INB-07) — IN PHIẾU NHẬP KHO CHÍNH THỨC (PNK) & BÀN GIAO

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-09 (INB-07)` |
| Use Case Name | In Phiếu Nhập Kho Chính Thức (PNK) & Bàn Giao |
| Module | `WMS / Inbound Print` |
...
Tự động xuất mẫu Phiếu Nhập Kho (PNK) khổ A4/A5 chuẩn Bộ Tài Chính có Barcode định danh (PNK-xxxx), chữ ký Thủ kho, Giao hàng, KCS và Kế toán trưởng để lưu trữ hồ sơ và bàn giao chứng từ.
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`GoodsReceiptPrintModal.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_WMS_INB07_GetReceiptDocumentPrint_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền và chứng từ ở trạng thái `STORED`.  
**When** Gửi yêu cầu tiếp nhận / cất kệ / nhập kho thành công.  
**Then** Trả 200 OK, CSDL chuyển sang `PRINTED / ARCHIVED`, ghi Sổ Cái Kép và phát âm thanh phản hồi.
...
### 6.1. Frontend — React (`GoodsReceiptPrintModal.tsx`)
...
- **Stored Procedure:** `api.usp_WMS_INB07_GetReceiptDocumentPrint_v1`
...
sequenceDiagram
    autonumber
    actor User as Thủ kho / Kế toán kho
    participant UI as React UI (GoodsReceiptPrintModal.tsx)
    ...
    API->>DB: 5. EXEC api.usp_WMS_INB07_GetReceiptDocumentPrint_v1 @UserId, @Params
    
    activate DB
    Note over DB: BƯỚC 1: SET XACT_ABORT ON & Kiểm tra quyền
    Note over DB: BƯỚC 2: BEGIN TRANSACTION & Khóa dữ liệu mục tiêu (UPDLOCK)
    Note over DB: BƯỚC 3: Kiểm tra điều kiện nghiệp vụ (STORED)
...
```mermaid
flowchart TD
    User["Thủ kho / Kế toán kho"]
    ReactUI["React UI (GoodsReceiptPrintModal.tsx)"]
    BackendAPI["Backend API (.NET 8)"]
    AuthCheck{"Token hợp lệ & Có quyền?"}
    StatusCheck{"Trạng thái hợp lệ (STORED)?"}
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_INB07_GetReceiptDocumentPrint_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật PRINTED / ARCHIVED"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `PRINTED / ARCHIVED` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_WMS_INB07_GetReceiptDocumentPrint_v1` chuẩn Transaction.
