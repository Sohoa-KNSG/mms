# UC-24 (OUT-05) — PHÊ DUYỆT ĐỀ NGHỊ XUẤT KHO ĐA CẤP

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-24 (OUT-05)` |
| Use Case Name | Phê Duyệt Đề Nghị Xuất Kho Đa Cấp |
| Module | `WMS / Outbound Approval` |
...
Cung cấp quy trình phê duyệt điện tử đa cấp cho phiếu đề nghị xuất kho: Cấp 1 (Quản đốc phân xưởng duyệt chuyển trạng thái sang 3 hoặc 4), Cấp 2 (Ban Giám Đốc duyệt các đơn vượt/ngoài định mức chuyển sang 4 hoặc 5). Hỗ trợ ký duyệt nhanh theo lô trên Web và Mobile.

### 1.2. Primary Actor
- **Quản đốc phân xưởng / Ban Giám Đốc**
...
| `BR-${doc.id.replace(/[^a-zA-Z0-9]/g, '')}-03` | Status Gate | Dữ liệu mục tiêu phải ở trạng thái hợp lệ (`PENDING_APPROVAL (1)`). | `409 Conflict` |
...
### 3.1. Main Flow
1. Người dùng mở màn hình chức năng trên giao diện (`OutboundApprovalPage.tsx`).
...
6. Backend kiểm tra Middleware Auth, kiểm tra Business Rules và gọi Stored Procedure `api.usp_WMS_OUT05_ApproveIssueRequest_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền hợp lệ và dữ liệu ở trạng thái `PENDING_APPROVAL (1)`.  
**When** Người dùng gửi yêu cầu xử lý thành công.  
**Then** Hệ thống trả về `200 OK`, CSDL cập nhật sang `APPROVED (4 / 5)`, ghi nhật ký Audit Log và phát âm thanh phản hồi.
...
- Component: `OutboundApprovalPage.tsx`
...
### 6.1. Frontend — React (`OutboundApprovalPage.tsx`)
...
- Thin API Endpoint nhận DTO, trích xuất Claim và ủy thác cho `api.usp_WMS_OUT05_ApproveIssueRequest_v1`.

### 6.3. API Contract & Stored Procedure
- **Endpoint:** `POST /api/v1/...`
- **Stored Procedure:** `api.usp_WMS_OUT05_ApproveIssueRequest_v1`
...
|---|---|---|---|---|
| `dbo.tbl_phieu_yeucau` | `trang_thai` | `PENDING_APPROVAL (1)` | `APPROVED (4 / 5)` | Chuyển đổi trạng thái nghiệp vụ thành công |
...
sequenceDiagram
    autonumber
    actor User as Quản đốc phân xưởng / Ban Giám Đốc
    participant UI as React UI (OutboundApprovalPage.tsx)
    ...
    API->>DB: 5. EXEC api.usp_WMS_OUT05_ApproveIssueRequest_v1 @UserId, @Params
    
    activate DB
    Note over DB: BƯỚC 1: SET XACT_ABORT ON & Kiểm tra quyền
    Note over DB: BƯỚC 2: BEGIN TRANSACTION & Khóa dữ liệu mục tiêu (UPDLOCK)
    Note over DB: BƯỚC 3: Kiểm tra trạng thái hợp lệ (PENDING_APPROVAL (1))
...
```mermaid
flowchart TD
    User["Quản đốc phân xưởng / Ban Giám Đốc"]
    ReactUI["React UI (OutboundApprovalPage.tsx)"]
    BackendAPI["Backend API (.NET 8)"]
    AuthCheck{"Token hợp lệ & Có quyền?"}
    StatusCheck{"Trạng thái hợp lệ (PENDING_APPROVAL (1))?"}
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_OUT05_ApproveIssueRequest_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật APPROVED (4 / 5)"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `APPROVED (4 / 5)` | `AC-01` |
| `TC-02` | Sai trạng thái | Dữ liệu không ở trạng thái `PENDING_APPROVAL (1)` | Gửi request | Trả 409 Conflict, không đổi DB | `AC-02` |
...
- [x] API Contract & Stored Procedure `api.usp_WMS_OUT05_ApproveIssueRequest_v1` chuẩn Transaction.
