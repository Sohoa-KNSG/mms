# UC-22 (OUT-07) — QUÉT BARCODE & SOẠN HÀNG THEO LÔ (BATCH) TRÊN PDA

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-22 (OUT-07)` |
| Use Case Name | Quét Barcode & Soạn Hàng Theo Lô (Batch) Trên PDA |
| Module | `WMS / Outbound Picking` |
...
Hướng dẫn nhân viên kho di chuyển chính xác đến từng vị trí Ô kệ theo lộ trình tối ưu, quét Barcode Lô hàng (Batch ID), kiểm tra khớp mã SKU, trạng thái QC Pass, nhập số lượng lấy thực tế và trừ tồn kho vật lý tức thời vào Sổ Cái Kép.

### 1.2. Primary Actor
- **Nhân viên lấy hàng PDA**
...
### 3.1. Main Flow
1. Người dùng mở màn hình chức năng trên giao diện (`HandheldPickingScanView.tsx`).
...
6. Backend kiểm tra Middleware Auth, kiểm tra Business Rules và gọi Stored Procedure `api.usp_WMS_OUT07_PickBatch_v1`.
...
**Then** Hệ thống trả về `200 OK`, CSDL cập nhật sang `PICKING (1) / PICKED (2)`, ghi nhật ký Audit Log và phát âm thanh phản hồi.
...
- Component: `HandheldPickingScanView.tsx`
...
### 6.1. Frontend — React (`HandheldPickingScanView.tsx`)
...
- Thin API Endpoint nhận DTO, trích xuất Claim và ủy thác cho `api.usp_WMS_OUT07_PickBatch_v1`.

### 6.3. API Contract & Stored Procedure
- **Endpoint:** `POST /api/v1/...`
- **Stored Procedure:** `api.usp_WMS_OUT07_PickBatch_v1`
...
|---|:---:|:---:|:---:|:---:|---|
| `dbo.tbl_batch_inv` | **X** | **X** | **X** | - | Bảng thực thể chính xử lý nghiệp vụ |
...
|---|---|---|---|---|
| `dbo.tbl_batch_inv` | `trang_thai` | `PICKING (1)` | `PICKING (1) / PICKED (2)` | Chuyển đổi trạng thái nghiệp vụ thành công |
...
sequenceDiagram
    autonumber
    actor User as Nhân viên lấy hàng PDA
    participant UI as React UI (HandheldPickingScanView.tsx)
    ...
    API->>DB: 5. EXEC api.usp_WMS_OUT07_PickBatch_v1 @UserId, @Params
...
```mermaid
flowchart TD
    User["Nhân viên lấy hàng PDA"]
    ReactUI["React UI (HandheldPickingScanView.tsx)"]
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_OUT07_PickBatch_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật PICKING (1) / PICKED (2)"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `PICKING (1) / PICKED (2)` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_WMS_OUT07_PickBatch_v1` chuẩn Transaction.
