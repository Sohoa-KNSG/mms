# UC-25 (RET-03) — KIỂM ĐẾM & XÁC NHẬN NHẬP TRẢ HÀNG VÀO KHO TRÊN PDA

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-25 (RET-03)` |
| Use Case Name | Kiểm Đếm & Xác Nhận Nhập Trả Hàng Vào Kho Trên PDA |
| Module | `WMS / PDA Return Receipt` |
...
Thủ kho dùng PDA quét kiểm đếm hàng trả lại, phân loại hàng còn tái sử dụng được vs Hàng phế liệu, cất vào Ô kệ quy định và chốt cộng tăng tồn kho vào Sổ Cái Kép.

### 1.2. Primary Actor
- **Thủ kho tiếp nhận trả hàng**
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`HandheldReturnScanView.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_WMS_RET03_ConfirmReturnPda_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền và chứng từ ở trạng thái `APPROVED_FOR_RETURN`.  
**When** Gửi yêu cầu tiếp nhận / cất kệ / nhập kho thành công.  
**Then** Trả 200 OK, CSDL chuyển sang `STORED_RETURN`, ghi Sổ Cái Kép và phát âm thanh phản hồi.
...
### 6.1. Frontend — React (`HandheldReturnScanView.tsx`)
...
- **Stored Procedure:** `api.usp_WMS_RET03_ConfirmReturnPda_v1`
...
sequenceDiagram
    autonumber
    actor User as Thủ kho tiếp nhận trả hàng
    participant UI as React UI (HandheldReturnScanView.tsx)
    ...
    API->>DB: 5. EXEC api.usp_WMS_RET03_ConfirmReturnPda_v1 @UserId, @Params
    
    activate DB
    Note over DB: BƯỚC 1: SET XACT_ABORT ON & Kiểm tra quyền
    Note over DB: BƯỚC 2: BEGIN TRANSACTION & Khóa dữ liệu mục tiêu (UPDLOCK)
    Note over DB: BƯỚC 3: Kiểm tra điều kiện nghiệp vụ (APPROVED_FOR_RETURN)
...
```mermaid
flowchart TD
    User["Thủ kho tiếp nhận trả hàng"]
    ReactUI["React UI (HandheldReturnScanView.tsx)"]
    BackendAPI["Backend API (.NET 8)"]
    AuthCheck{"Token hợp lệ & Có quyền?"}
    StatusCheck{"Trạng thái hợp lệ (APPROVED_FOR_RETURN)?"}
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_RET03_ConfirmReturnPda_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật STORED_RETURN"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `STORED_RETURN` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_WMS_RET03_ConfirmReturnPda_v1` chuẩn Transaction.
