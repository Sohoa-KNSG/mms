# UC-11 (LOC-04) — KHÓA & MỞ KHÓA VỊ TRÍ Ô KỆ PHỤC VỤ BẢO TRÌ & KIỂM KÊ

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-11 (LOC-04)` |
| Use Case Name | Khóa & Mở Khóa Vị Trí Ô Kệ Phục Vụ Bảo Trì & Kiểm Kê |
| Module | `WMS / Location Lock` |
...
Cho phép Khóa (Lock) hoặc Mở khóa (Unlock) một hoặc hàng loạt Ô kệ theo Dãy/Tầng. Khi bị khóa, hệ thống vô hiệu hóa việc cất hàng mới và lấy hàng từ ô kệ đó, đồng thời hiển thị biểu tượng ổ khóa trên sơ đồ 2D.
...
| `BR-${doc.id.replace(/[^a-zA-Z0-9]/g, '')}-03` | Status Gate | Dữ liệu mục tiêu phải ở trạng thái hợp lệ (`ACTIVE (1)`). | `409 Conflict` |
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`LocationLockModal.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_WMS_LOC04_SetLocationLock_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền và dữ liệu ở trạng thái `ACTIVE (1)`.  
**When** Gửi yêu cầu thực hiện thao tác thành công.  
**Then** Trả 200 OK, CSDL chuyển sang `LOCKED (0)`, ghi Audit Log và phát âm thanh phản hồi.
...
sequenceDiagram
    ...
    participant UI as React UI (LocationLockModal.tsx)
    ...
    API->>DB: 5. EXEC api.usp_WMS_LOC04_SetLocationLock_v1 @UserId, @Params
    
    activate DB
    Note over DB: BƯỚC 1: SET XACT_ABORT ON & Kiểm tra quyền
    Note over DB: BƯỚC 2: BEGIN TRANSACTION & Khóa dữ liệu mục tiêu (UPDLOCK)
    Note over DB: BƯỚC 3: Kiểm tra điều kiện nghiệp vụ (ACTIVE (1))
...
flowchart TD
    User["Quản lý kho"]
    ReactUI["React UI (LocationLockModal.tsx)"]
    BackendAPI["Backend API (.NET 8)"]
    AuthCheck{"Token hợp lệ & Có quyền?"}
    StatusCheck{"Trạng thái hợp lệ (ACTIVE (1))?"}
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_LOC04_SetLocationLock_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật LOCKED (0)"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `LOCKED (0)` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_WMS_LOC04_SetLocationLock_v1` chuẩn Transaction.
