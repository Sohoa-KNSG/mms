# UC-01 (AUTH-01) — ĐĂNG NHẬP, XÁC THỰC PHIÊN NGƯỜI DÙNG & QUẢN LÝ JWT COOKIE

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-01 (AUTH-01)` |
| Use Case Name | Đăng Nhập, Xác Thực Phiên Người Dùng & Quản Lý JWT Cookie |
| Module | `WMS / Authentication` |
...
Xác thực danh tính người dùng (Username/Password hoặc Thẻ RFID PDA). Khi hợp lệ, hệ thống sinh Access Token (JWT) lưu an toàn trong HttpOnly Cookie, thiết lập thời gian sống của phiên (8h), nạp ma trận phân quyền màn hình và trả về Profile người dùng.

### 1.2. Primary Actor
- **Toàn bộ Người dùng hệ thống MMS**
...
| `BR-${doc.id.replace(/[^a-zA-Z0-9]/g, '')}-03` | Status Gate | Dữ liệu mục tiêu phải ở trạng thái hợp lệ (`ANONYMOUS`). | `409 Conflict` |
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`LoginPage.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_SEC_Login_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền và dữ liệu ở trạng thái `ANONYMOUS`.  
**When** Gửi yêu cầu thực hiện thao tác thành công.  
**Then** Trả 200 OK, CSDL chuyển sang `AUTHENTICATED`, ghi Audit Log và phát âm thanh phản hồi.
...
|---|:---:|:---:|:---:|:---:|---|
| `dbo.tbl_dm_user` | **X** | **X** | **X** | - | Bảng thực thể chính xử lý nghiệp vụ |
...
sequenceDiagram
    autonumber
    actor User as Toàn bộ Người dùng hệ thống MMS
    participant UI as React UI (LoginPage.tsx)
...
```mermaid
flowchart TD
    User["Toàn bộ Người dùng hệ thống MMS"]
    ReactUI["React UI (LoginPage.tsx)"]
    BackendAPI["Backend API (.NET 8)"]
    AuthCheck{"Token hợp lệ & Có quyền?"}
    StatusCheck{"Trạng thái hợp lệ (ANONYMOUS)?"}
    ...
    ProcessLock -->|"5. Execute api.usp_SEC_Login_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật AUTHENTICATED"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `AUTHENTICATED` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_SEC_Login_v1` chuẩn Transaction.
