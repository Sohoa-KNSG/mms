# UC-06 (RET-02) — PHÊ DUYỆT PHIẾU NHẬP TRẢ HÀNG

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-06 (RET-02)` |
| Use Case Name | Phê Duyệt Phiếu Nhập Trả Hàng |
| Module | `WMS / Return Approval` |
...
Quản đốc thẩm tra lý do trả hàng và phê duyệt phiếu đề nghị trả hàng, chuyển trạng thái phiếu sang sẵn sàng nhận tại cửa kho.

### 1.2. Primary Actor
- **Quản đốc phân xưởng / Thủ kho**
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`ReturnApprovalPage.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_WMS_RET02_ApproveReturnRequest_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền và chứng từ ở trạng thái `PENDING_APPROVAL`.  
**When** Gửi yêu cầu tiếp nhận / cất kệ / nhập kho thành công.  
**Then** Trả 200 OK, CSDL chuyển sang `APPROVED_FOR_RETURN`, ghi Sổ Cái Kép và phát âm thanh phản hồi.
...
### 6.1. Frontend — React (`ReturnApprovalPage.tsx`)
...
- **Stored Procedure:** `api.usp_WMS_RET02_ApproveReturnRequest_v1`
...
sequenceDiagram
    autonumber
    actor User as Quản đốc phân xưởng / Thủ kho
    participant UI as React UI (ReturnApprovalPage.tsx)
    ...
    API->>DB: 5. EXEC api.usp_WMS_RET02_ApproveReturnRequest_v1 @UserId, @Params
    
    activate DB
    Note over DB: BƯỚC 1: SET XACT_ABORT ON & Kiểm tra quyền
    Note over DB: BƯỚC 2: BEGIN TRANSACTION & Khóa dữ liệu mục tiêu (UPDLOCK)
    Note over DB: BƯỚC 3: Kiểm tra điều kiện nghiệp vụ (PENDING_APPROVAL)
...
```mermaid
flowchart TD
    User["Quản đốc phân xưởng / Thủ kho"]
    ReactUI["React UI (ReturnApprovalPage.tsx)"]
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_RET02_ApproveReturnRequest_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật APPROVED_FOR_RETURN"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `APPROVED_FOR_RETURN` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_WMS_RET02_ApproveReturnRequest_v1` chuẩn Transaction.
