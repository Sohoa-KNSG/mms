# UC-06 (RET-01) — ĐĂNG KÝ ĐỀ NGHỊ NHẬP TRẢ HÀNG & PHẾ LIỆU TỪ PHÂN XƯỞNG

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-06 (RET-01)` |
| Use Case Name | Đăng Ký Đề Nghị Nhập Trả Hàng & Phế Liệu Từ Phân Xưởng |
| Module | `WMS / Return Request` |
...
Cho phép phân xưởng lập phiếu đề nghị trả lại kho các vật tư thừa sau sản xuất, phế liệu thu hồi hoặc vật tư không đạt yêu cầu kỹ thuật (nghiep_vu = RET_PROD).
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`ReturnRequestCreatePage.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_WMS_RET01_CreateReturnRequest_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền và chứng từ ở trạng thái `DRAFT`.  
**When** Gửi yêu cầu tiếp nhận / cất kệ / nhập kho thành công.  
**Then** Trả 200 OK, CSDL chuyển sang `PENDING_APPROVAL`, ghi Sổ Cái Kép và phát âm thanh phản hồi.
...
### 6.1. Frontend — React (`ReturnRequestCreatePage.tsx`)
...
- **Stored Procedure:** `api.usp_WMS_RET01_CreateReturnRequest_v1`
...
|---|:---:|:---:|:---:|:---:|---|
| `dbo.tbl_phieu_yeucau` | **X** | **X** | **X** | - | Bảng thực thể chính xử lý nhập kho / trả hàng |
...
sequenceDiagram
    autonumber
    actor User as Nhân viên phân xưởng
    participant UI as React UI (ReturnRequestCreatePage.tsx)
    ...
    API->>DB: 5. EXEC api.usp_WMS_RET01_CreateReturnRequest_v1 @UserId, @Params
...
```mermaid
flowchart TD
    User["Nhân viên phân xưởng"]
    ReactUI["React UI (ReturnRequestCreatePage.tsx)"]
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_RET01_CreateReturnRequest_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật PENDING_APPROVAL"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `PENDING_APPROVAL` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_WMS_RET01_CreateReturnRequest_v1` chuẩn Transaction.
