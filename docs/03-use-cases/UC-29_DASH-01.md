# UC-29 (DASH-01) — BẢNG ĐIỀU KHIỂN TỔNG QUAN DASHBOARD & MÀN HÌNH TIVI GIÁM SÁT REALTIME

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-29 (DASH-01)` |
| Use Case Name | Bảng Điều Khiển Tổng Quan Dashboard & Màn Hình Tivi Giám Sát Realtime |
| Module | `WMS / Executive Dashboard & TV Wallboard` |
...
Cung cấp trung tâm điều hành trực quan hóa hoạt động kho hàng realtime: 7 KPI Nhập kho, 7 KPI Xuất kho, Bảng 1 (Danh sách phiếu chờ soạn hàng xuất kho) và Bảng 2 (Danh sách phiếu đã soạn chờ xưởng nhận) với cơ chế tự động làm mới 30 giây.

### 1.2. Primary Actor
- **Ban Giám Đốc / Quản Đốc / Thủ Kho**
...
| `BR-${doc.id.replace(/[^a-zA-Z0-9]/g, '')}-03` | Status Gate | Dữ liệu mục tiêu phải ở trạng thái hợp lệ (`REALTIME`). | `409 Conflict` |
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`TvDashboardPage.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_WMS_DASH01_GetTvMetrics_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền và dữ liệu ở trạng thái `REALTIME`.  
**When** Gửi yêu cầu thực hiện thao tác thành công.  
**Then** Trả 200 OK, CSDL chuyển sang `SYNCHRONIZED`, ghi Audit Log và phát âm thanh phản hồi.
...
sequenceDiagram
    autonumber
    actor User as Ban Giám Đốc / Quản Đốc / Thủ Kho
    participant UI as React UI (TvDashboardPage.tsx)
    ...
    API->>DB: 5. EXEC api.usp_WMS_DASH01_GetTvMetrics_v1 @UserId, @Params
    
    activate DB
    Note over DB: BƯỚC 1: SET XACT_ABORT ON & Kiểm tra quyền
    Note over DB: BƯỚC 2: BEGIN TRANSACTION & Khóa dữ liệu mục tiêu (UPDLOCK)
    Note over DB: BƯỚC 3: Kiểm tra điều kiện nghiệp vụ (REALTIME)
...
```mermaid
flowchart TD
    User["Ban Giám Đốc / Quản Đốc / Thủ Kho"]
    ReactUI["React UI (TvDashboardPage.tsx)"]
    BackendAPI["Backend API (.NET 8)"]
    AuthCheck{"Token hợp lệ & Có quyền?"}
    StatusCheck{"Trạng thái hợp lệ (REALTIME)?"}
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_DASH01_GetTvMetrics_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật SYNCHRONIZED"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `SYNCHRONIZED` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_WMS_DASH01_GetTvMetrics_v1` chuẩn Transaction.
