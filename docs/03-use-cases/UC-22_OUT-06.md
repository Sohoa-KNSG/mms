---
title: "Đặc tả OUT-06 - Lập danh sách soạn hàng"
use_case_id: "OUT-06"
version: "1.0"
date: "2026-08-13"
...
| Màn hình quyền | `scr_soanhang`, `scr_soanhang_chitiet` |
...
Chuyển phiếu đã duyệt thành nhiệm vụ soạn.
...
| Thành công | Hoàn tất đúng luồng 'OUT-06', trả dữ liệu/kết quả contract và ghi audit khi có command. |
...
2. tải chi tiết.
3. phân bổ tồn.
4. tạo danh sách soạn..

### 1.4. Ngoại lệ và kiểm soát

Thiếu tồn: hiển thị thiếu hụt và không xác nhận đủ.

### 1.5. Business Rules

| Mã | Quy tắc |
| --- | --- |
| BR-OUT-06-01 | User phải có phiên xác thực và quyền màn hình tương ứng. |
| BR-OUT-06-02 | Dữ liệu bắt buộc phải được trim và kiểm tra tại API lẫn stored procedure. |
| BR-OUT-06-03 | Không tin 'UserId', trạng thái hoặc giá trị suy diễn do client tự gửi. |
| BR-OUT-06-04 | Stored procedure là nơi thực thi logic nghiệp vụ và phân quyền dữ liệu. |
| BR-OUT-06-05 | Command phải nguyên tử, rollback khi bất kỳ bước nào lỗi. |
| BR-OUT-06-06 | Giữ nguyên cấu trúc bảng và mã trạng thái legacy. |
| BR-OUT-06-07 | Lỗi nghiệp vụ phải dùng mã ổn định; lỗi ngoài dự kiến phải có 'traceId'. |
| BR-OUT-06-08 | React không truy cập bảng SQL trực tiếp. |
...
- Header hiển thị mã 'OUT-06' và tên “Lập danh sách soạn hàng”.
...
| Screen | Route |
| --- | --- |
| scr_soanhang | /outbound/picking |
| scr_soanhang_chitiet | /outbound/picking |

### 3.2. .NET endpoint contract

| Endpoint name | Vai trò |
| --- | --- |
| `OUT-06_GetPickingQueue` | .NET endpoint đã định danh |
| `OUT-06_GetPickingRequest` | .NET endpoint đã định danh |
| `OUT-06_StartPicking` | .NET endpoint đã định danh |
...
| --- | --- |
| `api.usp_WMS_OUT06_GetPickingQueue_v1` | `database/stored-procedures/w6/api.usp_WMS_OUT06_GetPickingQueue_v1.sql` |
| `api.usp_WMS_OUT06_GetPickingRequest_v1` | `database/stored-procedures/w6/api.usp_WMS_OUT06_GetPickingRequest_v1.sql` |
| `api.usp_WMS_OUT06_StartPicking_v1` | `database/stored-procedures/w6/api.usp_WMS_OUT06_StartPicking_v1.sql` |

**api.usp_WMS_OUT06_GetPickingQueue_v1**

~~~sql
api.usp_WMS_OUT06_GetPickingQueue_v1 @UserId nvarchar(50), @Search nvarchar(200) = NULL, @Status nvarchar(20) = NULL, @Page int = 1, @PageSize int = 50
~~~

**api.usp_WMS_OUT06_GetPickingRequest_v1**

~~~sql
api.usp_WMS_OUT06_GetPickingRequest_v1 @UserId nvarchar(50), @RequestId int
~~~

**api.usp_WMS_OUT06_StartPicking_v1**

~~~sql
api.usp_WMS_OUT06_StartPicking_v1 @UserId nvarchar(50), @RequestId int
~~~
...
| --- | --- | --- |
| 51002 | 400 | Trang thai soan hang khong hop le. |
| 51001 | 403 | Khong co quyen xem hang doi soan hang. |
| 51001 | 403 | Khong co quyen xem chi tiet soan hang. |
| 51001 | 403 | Khong co quyen bat dau soan hang. |
| 51001 | 403 | Tai khoan khong co ma nhan vien. |
| 51004 | 404 | Phieu khong o trang thai cho hoac dang soan. |
...
| --- | --- | --- |
| `v_yeucau_soanhang, vw_status_soan_vattu` | Dữ liệu nghiệp vụ legacy | SP |
...
sequenceDiagram
    autonumber
    actor User
    participant UI as React OUT-06
    participant API as .NET API
    participant SP as SQL Contract
    participant DB as MMS Legacy Tables
    User->>UI: Thực hiện Lập danh sách soạn hàng
...
flowchart TD
    ...
    S1["tải chi tiết"]
    S2["phân bổ tồn"]
    S3["tạo danh sách soạn."]
    S0 --> S1
    S1 --> S2
    S2 --> S3
    S3 --> V{"Hợp lệ?"}
    V -- Có --> OK["Hoàn tất OUT-06"]
...
| --- | --- | --- |
| AC-OUT-06-01 | User có quyền mở use case | Màn hình và dữ liệu đúng phạm vi được hiển thị |
| AC-OUT-06-02 | User không có quyền | HTTP 403, không lộ dữ liệu |
| AC-OUT-06-03 | Dữ liệu hợp lệ | Hoàn tất đúng luồng chính |
| AC-OUT-06-04 | Thiếu dữ liệu bắt buộc | HTTP 400/422, chỉ rõ lỗi |
| AC-OUT-06-05 | Đối tượng không tồn tại | HTTP 404, không ghi dở dang |
| AC-OUT-06-06 | Dữ liệu đã thay đổi đồng thời | HTTP 409 hoặc kết quả nhất quán |
| AC-OUT-06-07 | Lỗi hệ thống | HTTP 500 với 'traceId' |
| AC-OUT-06-08 | Kiểm tra audit | User, thời gian và hành động đúng contract |
| AC-OUT-06-09 | Kiểm tra phân quyền dữ liệu | Không thấy dữ liệu ngoài phạm vi |
| AC-OUT-06-10 | Kiểm tra Power Apps dự phòng | Bảng và trạng thái legacy vẫn tương thích |
...
| Nguồn | Tham chiếu |
| --- | --- |
| Hồ sơ nghiệp vụ | 'HO_SO_TONG_THE_UNG_DUNG_MMS.md' – OUT-06 |
| Route registry | 'apps/web/src/app/routeRegistry.ts' |
| API source | apps/api/Modules/OutboundPicking/OutboundPickingEndpoints.cs |
| SQL source | database/stored-procedures/w6/api.usp_WMS_OUT06_GetPickingQueue_v1.sql, database/stored-procedures/w6/api.usp_WMS_OUT06_GetPickingRequest_v1.sql, database/stored-procedures/w6/api.usp_WMS_OUT06_StartPicking_v1.sql |
