# UC-08 (INB-06) — XÁC NHẬN NHẬP KHO CHÍNH THỨC & HẠCH TOÁN SỔ CÁI KÉP

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-08 (INB-06)` |
| Use Case Name | Xác Nhận Nhập Kho Chính Thức & Hạch Toán Sổ Cái Kép |
| Module | `WMS / Official Inbound Close` |
...
Sau khi toàn bộ các Lô hàng trong đơn PO đã được kiểm định QC Pass và cất vào Ô kệ (ON_RACK), Thủ kho kiểm tra đối chiếu và bấm duyệt nhập kho chính thức. Hệ thống cập nhật status_kho = STORED, sinh tồn khả dụng tbl_batch_inv, chốt số lượng PO Bravo và hạch toán Nợ/Có vào Sổ Cái Kép.
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`ReceivingConfirmOfficialModal.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_WMS_INB06_ConfirmOfficialReceipt_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền và chứng từ ở trạng thái `ON_RACK`.  
**When** Gửi yêu cầu tiếp nhận / cất kệ / nhập kho thành công.  
**Then** Trả 200 OK, CSDL chuyển sang `STORED (OFFICIAL_POSTED)`, ghi Sổ Cái Kép và phát âm thanh phản hồi.
...
### 6.1. Frontend — React (`ReceivingConfirmOfficialModal.tsx`)
...
- **Stored Procedure:** `api.usp_WMS_INB06_ConfirmOfficialReceipt_v1`
...
|---|:---:|:---:|:---:|:---:|---|
| `dbo.tbl_phieu_transaction` | **X** | **X** | **X** | - | Bảng thực thể chính xử lý nhập kho / trả hàng |
...
sequenceDiagram
    ...
    participant UI as React UI (ReceivingConfirmOfficialModal.tsx)
    ...
    API->>DB: 5. EXEC api.usp_WMS_INB06_ConfirmOfficialReceipt_v1 @UserId, @Params
    
    activate DB
    Note over DB: BƯỚC 1: SET XACT_ABORT ON & Kiểm tra quyền
    Note over DB: BƯỚC 2: BEGIN TRANSACTION & Khóa dữ liệu mục tiêu (UPDLOCK)
    Note over DB: BƯỚC 3: Kiểm tra điều kiện nghiệp vụ (ON_RACK)
...
flowchart TD
    ...
    ReactUI["React UI (ReceivingConfirmOfficialModal.tsx)"]
    BackendAPI["Backend API (.NET 8)"]
    AuthCheck{"Token hợp lệ & Có quyền?"}
    StatusCheck{"Trạng thái hợp lệ (ON_RACK)?"}
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_INB06_ConfirmOfficialReceipt_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật STORED (OFFICIAL_POSTED)"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `STORED (OFFICIAL_POSTED)` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_WMS_INB06_ConfirmOfficialReceipt_v1` chuẩn Transaction.
