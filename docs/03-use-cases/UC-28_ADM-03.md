# UC-28 (ADM-03) — GIÁM SÁT PHIÊN HOẠT ĐỘNG & NHẬT KÝ BẢO MẬT (AUDIT LOGS)

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-28 (ADM-03)` |
| Use Case Name | Giám Sát Phiên Hoạt Động & Nhật Ký Bảo Mật (Audit Logs) |
| Module | `WMS / Security Audit Logs` |
...
Ghi vết và giám sát toàn bộ các hành động trọng yếu trên hệ thống: Đăng nhập/đăng xuất, thay đổi mật khẩu, xóa/hủy phiếu, điều chỉnh số liệu kiểm kê, xuất kho vượt định mức. Cho phép tra cứu theo thời gian, IP, UserId và mã chức năng.
...
| `BR-${doc.id.replace(/[^a-zA-Z0-9]/g, '')}-03` | Status Gate | Dữ liệu mục tiêu phải ở trạng thái hợp lệ (`LOGGED`). | `409 Conflict` |
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`AdminAuditLogViewerPage.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_ADM_GetAuditLogs_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền và dữ liệu ở trạng thái `LOGGED`.  
**When** Gửi yêu cầu thực hiện thao tác thành công.  
**Then** Trả 200 OK, CSDL chuyển sang `VIEWED`, ghi Audit Log và phát âm thanh phản hồi.
...
|---|:---:|:---:|:---:|:---:|---|
| `dbo.tbl_sec_audit_log` | **X** | **X** | **X** | - | Bảng thực thể chính xử lý nghiệp vụ |
...
sequenceDiagram
    ...
    participant UI as React UI (AdminAuditLogViewerPage.tsx)
...
flowchart TD
    ...
    ReactUI["React UI (AdminAuditLogViewerPage.tsx)"]
    BackendAPI["Backend API (.NET 8)"]
    AuthCheck{"Token hợp lệ & Có quyền?"}
    StatusCheck{"Trạng thái hợp lệ (LOGGED)?"}
    ...
    ProcessLock -->|"5. Execute api.usp_ADM_GetAuditLogs_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật VIEWED"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `VIEWED` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_ADM_GetAuditLogs_v1` chuẩn Transaction.
