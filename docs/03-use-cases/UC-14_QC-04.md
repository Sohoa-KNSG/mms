# UC-14 (QC-04) — PHÊ DUYỆT KẾT LUẬN QC PASS / QC REJECT & PHÂN LOẠI XỬ LÝ LÔ

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-14 (QC-04)` |
| Use Case Name | Phê Duyệt Kết Luận QC Pass / QC Reject & Phân Loại Xử Lý Lô |
| Module | `WMS / QC Approval` |
...
Căn cứ trên kết quả đo lường và quy tắc chấp nhận AQL, Trưởng bộ phận QC đưa ra quyết định chính thức: QC PASS (giải phóng Lô hàng cất kệ/xuất kho), QC REJECT (khóa Lô chuyển vào kho cách ly) hoặc CONCESSION (nhân nhượng có điều kiện).

### 1.2. Primary Actor
- **Trưởng bộ phận KCS / QC**
...
| `BR-${doc.id.replace(/[^a-zA-Z0-9]/g, '')}-03` | Status Gate | Dữ liệu mục tiêu phải ở trạng thái hợp lệ (`MEASURED`). | `409 Conflict` |
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`QcDecisionApprovalPage.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_WMS_QC04_SubmitInspectionDecision_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền và dữ liệu ở trạng thái `MEASURED`.  
**When** Gửi yêu cầu thực hiện thao tác thành công.  
**Then** Trả 200 OK, CSDL chuyển sang `PASS / REJECT`, ghi Audit Log và phát âm thanh phản hồi.
...
|---|:---:|:---:|:---:|:---:|---|
| `dbo.tbl_qc_inspection` | **X** | **X** | **X** | - | Bảng thực thể chính xử lý nghiệp vụ |
...
sequenceDiagram
    autonumber
    actor User as Trưởng bộ phận KCS / QC
    participant UI as React UI (QcDecisionApprovalPage.tsx)
    ...
    API->>DB: 5. EXEC api.usp_WMS_QC04_SubmitInspectionDecision_v1 @UserId, @Params
    
    activate DB
    Note over DB: BƯỚC 1: SET XACT_ABORT ON & Kiểm tra quyền
    Note over DB: BƯỚC 2: BEGIN TRANSACTION & Khóa dữ liệu mục tiêu (UPDLOCK)
    Note over DB: BƯỚC 3: Kiểm tra điều kiện nghiệp vụ (MEASURED)
...
```mermaid
flowchart TD
    User["Trưởng bộ phận KCS / QC"]
    ReactUI["React UI (QcDecisionApprovalPage.tsx)"]
    BackendAPI["Backend API (.NET 8)"]
    AuthCheck{"Token hợp lệ & Có quyền?"}
    StatusCheck{"Trạng thái hợp lệ (MEASURED)?"}
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_QC04_SubmitInspectionDecision_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật PASS / REJECT"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `PASS / REJECT` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_WMS_QC04_SubmitInspectionDecision_v1` chuẩn Transaction.
