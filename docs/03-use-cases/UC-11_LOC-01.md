# UC-11 (LOC-01) — QUẢN LÝ DANH MỤC 540 VỊ TRÍ Ô KỆ KHO

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-11 (LOC-01)` |
| Use Case Name | Quản Lý Danh Mục 540 Vị Trí Ô Kệ Kho |
| Module | `WMS / Location Management` |
...
Quản lý toàn bộ 540 vị trí Ô kệ vật lý trong kho theo cấu trúc mã hóa [Khu Vực][Dãy]-[Tầng]-[Cột]. Cho phép định nghĩa thông số: Loại kệ, Kích thước, Tải trọng tối đa (kg), Thể tích tối đa (m3) và Nhóm SKU ưu tiên.

### 1.2. Primary Actor
- **Quản trị hệ thống / Quản lý kho**
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`LocationCatalogPage.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_WMS_LOC01_GetLocations_v1`.
...
sequenceDiagram
    autonumber
    actor User as Quản trị hệ thống / Quản lý kho
    participant UI as React UI (LocationCatalogPage.tsx)
    ...
    API->>DB: 5. EXEC api.usp_WMS_LOC01_GetLocations_v1 @UserId, @Params
...
```mermaid
flowchart TD
    User["Quản trị hệ thống / Quản lý kho"]
    ReactUI["React UI (LocationCatalogPage.tsx)"]
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_LOC01_GetLocations_v1"| DB
...
- [x] API Contract & Stored Procedure `api.usp_WMS_LOC01_GetLocations_v1` chuẩn Transaction.
