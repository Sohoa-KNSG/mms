# UC-21 (OUT-04) — TIẾP NHẬN & THẨM TRA DANH SÁCH ĐỀ NGHỊ XUẤT KHO

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-21 (OUT-04)` |
| Use Case Name | Tiếp Nhận & Thẩm Tra Danh Sách Đề Nghị Xuất Kho |
| Module | `WMS / Outbound Review` |
...
Cung cấp danh sách tổng hợp toàn bộ các phiếu đề nghị xuất kho từ các phân xưởng gửi về. Cho phép Thủ kho kiểm tra tính khả thi tồn kho, đối chiếu định mức, xem trước vị trí Ô kệ và phản hồi khả năng đáp ứng trước khi chuyển Quản đốc duyệt.

### 1.2. Primary Actor
- **Thủ kho xuất hàng / Kế toán kho**
...
| `BR-${doc.id.replace(/[^a-zA-Z0-9]/g, '')}-03` | Status Gate | Dữ liệu mục tiêu phải ở trạng thái hợp lệ (`PENDING_APPROVAL`). | `409 Conflict` |
...
### 3.1. Main Flow
1. Người dùng mở màn hình chức năng trên giao diện (`OutboundReviewQueuePage.tsx`).
...
6. Backend kiểm tra Middleware Auth, kiểm tra Business Rules và gọi Stored Procedure `api.usp_WMS_OUT04_ReviewIssueRequests_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền hợp lệ và dữ liệu ở trạng thái `PENDING_APPROVAL`.  
**When** Người dùng gửi yêu cầu xử lý thành công.  
**Then** Hệ thống trả về `200 OK`, CSDL cập nhật sang `REVIEWED`, ghi nhật ký Audit Log và phát âm thanh phản hồi.
...
- Component: `OutboundReviewQueuePage.tsx`
...
### 6.1. Frontend — React (`OutboundReviewQueuePage.tsx`)
...
- Thin API Endpoint nhận DTO, trích xuất Claim và ủy thác cho `api.usp_WMS_OUT04_ReviewIssueRequests_v1`.

### 6.3. API Contract & Stored Procedure
- **Endpoint:** `POST /api/v1/...`
- **Stored Procedure:** `api.usp_WMS_OUT04_ReviewIssueRequests_v1`
...
|---|---|---|---|---|
| `dbo.tbl_phieu_yeucau` | `trang_thai` | `PENDING_APPROVAL` | `REVIEWED` | Chuyển đổi trạng thái nghiệp vụ thành công |
...
sequenceDiagram
    autonumber
    actor User as Thủ kho xuất hàng / Kế toán kho
    participant UI as React UI (OutboundReviewQueuePage.tsx)
    ...
    API->>DB: 5. EXEC api.usp_WMS_OUT04_ReviewIssueRequests_v1 @UserId, @Params
    
    activate DB
    Note over DB: BƯỚC 1: SET XACT_ABORT ON & Kiểm tra quyền
    Note over DB: BƯỚC 2: BEGIN TRANSACTION & Khóa dữ liệu mục tiêu (UPDLOCK)
    Note over DB: BƯỚC 3: Kiểm tra trạng thái hợp lệ (PENDING_APPROVAL)
...
```mermaid
flowchart TD
    User["Thủ kho xuất hàng / Kế toán kho"]
    ReactUI["React UI (OutboundReviewQueuePage.tsx)"]
    BackendAPI["Backend API (.NET 8)"]
    AuthCheck{"Token hợp lệ & Có quyền?"}
    StatusCheck{"Trạng thái hợp lệ (PENDING_APPROVAL)?"}
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_OUT04_ReviewIssueRequests_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật REVIEWED"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `REVIEWED` | `AC-01` |
| `TC-02` | Sai trạng thái | Dữ liệu không ở trạng thái `PENDING_APPROVAL` | Gửi request | Trả 409 Conflict, không đổi DB | `AC-02` |
...
- [x] API Contract & Stored Procedure `api.usp_WMS_OUT04_ReviewIssueRequests_v1` chuẩn Transaction.
