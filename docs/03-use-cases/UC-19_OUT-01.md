# UC-19 (OUT-01) — ĐĂNG KÝ ĐỀ NGHỊ XUẤT KHO THEO ĐỊNH MỨC BOM

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-19 (OUT-01)` |
| Use Case Name | Đăng Ký Đề Nghị Xuất Kho Theo Định Mức BOM |
| Module | `WMS / Outbound Request` |
...
Cho phép nhân viên phân xưởng đăng ký phiếu đề nghị xuất vật tư phục vụ lệnh sản xuất căn cứ theo Định Mức Kỹ Thuật (Bill of Materials - BOM). Hệ thống tự động tính toán nhu cầu vật tư dựa trên sản lượng thành phẩm cần sản xuất và đối chiếu tồn kho khả dụng trước khi chuyển sang luồng phê duyệt.

### 1.2. Primary Actor
- **Nhân viên kế hoạch / Quản đốc phân xưởng**
...
| `BR-${doc.id.replace(/[^a-zA-Z0-9]/g, '')}-03` | Status Gate | Dữ liệu mục tiêu phải ở trạng thái hợp lệ (`DRAFT`). | `409 Conflict` |
...
### 3.1. Main Flow
1. Người dùng mở màn hình chức năng trên giao diện (`OutboundBomRequestPage.tsx`).
...
6. Backend kiểm tra Middleware Auth, kiểm tra Business Rules và gọi Stored Procedure `api.usp_WMS_OUT01_CreateBomIssueRequest_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền hợp lệ và dữ liệu ở trạng thái `DRAFT`.  
**When** Người dùng gửi yêu cầu xử lý thành công.  
**Then** Hệ thống trả về `200 OK`, CSDL cập nhật sang `PENDING_APPROVAL (trang_thai_phieu = 1)`, ghi nhật ký Audit Log và phát âm thanh phản hồi.
...
- Component: `OutboundBomRequestPage.tsx`
...
### 6.1. Frontend — React (`OutboundBomRequestPage.tsx`)
...
- Thin API Endpoint nhận DTO, trích xuất Claim và ủy thác cho `api.usp_WMS_OUT01_CreateBomIssueRequest_v1`.

### 6.3. API Contract & Stored Procedure
- **Endpoint:** `POST /api/v1/...`
- **Stored Procedure:** `api.usp_WMS_OUT01_CreateBomIssueRequest_v1`
...
|---|---|---|---|---|
| `dbo.tbl_phieu_yeucau` | `trang_thai` | `DRAFT` | `PENDING_APPROVAL (trang_thai_phieu = 1)` | Chuyển đổi trạng thái nghiệp vụ thành công |
...
sequenceDiagram
    autonumber
    actor User as Nhân viên kế hoạch / Quản đốc phân xưởng
    participant UI as React UI (OutboundBomRequestPage.tsx)
    ...
    API->>DB: 5. EXEC api.usp_WMS_OUT01_CreateBomIssueRequest_v1 @UserId, @Params
    
    activate DB
    Note over DB: BƯỚC 1: SET XACT_ABORT ON & Kiểm tra quyền
    Note over DB: BƯỚC 2: BEGIN TRANSACTION & Khóa dữ liệu mục tiêu (UPDLOCK)
    Note over DB: BƯỚC 3: Kiểm tra trạng thái hợp lệ (DRAFT)
...
```mermaid
flowchart TD
    User["Nhân viên kế hoạch / Quản đốc phân xưởng"]
    ReactUI["React UI (OutboundBomRequestPage.tsx)"]
    BackendAPI["Backend API (.NET 8)"]
    AuthCheck{"Token hợp lệ & Có quyền?"}
    StatusCheck{"Trạng thái hợp lệ (DRAFT)?"}
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_OUT01_CreateBomIssueRequest_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật PENDING_APPROVAL (trang_thai_phieu = 1)"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `PENDING_APPROVAL (trang_thai_phieu = 1)` | `AC-01` |
| `TC-02` | Sai trạng thái | Dữ liệu không ở trạng thái `DRAFT` | Gửi request | Trả 409 Conflict, không đổi DB | `AC-02` |
...
- [x] API Contract & Stored Procedure `api.usp_WMS_OUT01_CreateBomIssueRequest_v1` chuẩn Transaction.
