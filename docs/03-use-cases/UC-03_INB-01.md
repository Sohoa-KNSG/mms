# UC-03 (INB-01) — TIẾP NHẬN ĐƠN HÀNG NHẬP KHO THEO PO BRAVO

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-03 (INB-01)` |
| Use Case Name | Tiếp Nhận Đơn Hàng Nhập Kho Theo PO Bravo |
| Module | `WMS / Inbound Receiving` |
...
Đồng bộ và tiếp nhận danh sách các Đơn mua hàng (PO) từ ERP Bravo. Cho phép nhân viên tra cứu theo số PO, đối chiếu danh mục vật tư đặt mua, quy cách, số lượng mở và mở phiên tiếp nhận kiểm đếm tại khu vực Staging Area.

### 1.2. Primary Actor
- **Nhân viên tiếp nhận kho / Thủ kho**
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`ReceivingPoOrderPage.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_WMS_INB01_GetPoOrders_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền và chứng từ ở trạng thái `OPEN_PO`.  
**When** Gửi yêu cầu tiếp nhận / cất kệ / nhập kho thành công.  
**Then** Trả 200 OK, CSDL chuyển sang `RECEIVING`, ghi Sổ Cái Kép và phát âm thanh phản hồi.
...
### 6.1. Frontend — React (`ReceivingPoOrderPage.tsx`)
...
- **Stored Procedure:** `api.usp_WMS_INB01_GetPoOrders_v1`
...
|---|:---:|:---:|:---:|:---:|---|
| `dbo.tbl_po_bravo` | **X** | **X** | **X** | - | Bảng thực thể chính xử lý nhập kho / trả hàng |
...
sequenceDiagram
    autonumber
    actor User as Nhân viên tiếp nhận kho / Thủ kho
    participant UI as React UI (ReceivingPoOrderPage.tsx)
...
```mermaid
flowchart TD
    User["Nhân viên tiếp nhận kho / Thủ kho"]
    ReactUI["React UI (ReceivingPoOrderPage.tsx)"]
    BackendAPI["Backend API (.NET 8)"]
    AuthCheck{"Token hợp lệ & Có quyền?"}
    StatusCheck{"Trạng thái hợp lệ (OPEN_PO)?"}
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_INB01_GetPoOrders_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật RECEIVING"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `RECEIVING` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_WMS_INB01_GetPoOrders_v1` chuẩn Transaction.
