# UC-17 (INV-03) — ĐIỀU CHUYỂN VỊ TRÍ TỒN KHO NỘI BỘ GIỮA CÁC Ô KỆ (LOCATION TRANSFER)

## 0. Document Control

| Thuộc tính | Nội dung |
|---|---|
| Use Case ID | `UC-17 (INV-03)` |
| Use Case Name | Điều Chuyển Vị Trí Tồn Kho Nội Bộ Giữa Các Ô Kệ (Location Transfer) |
| Module | `WMS / Inventory Transfer` |
...
Cho phép di chuyển một phần hoặc toàn bộ số lượng Lô hàng từ Ô kệ nguồn sang Ô kệ đích nhằm tối ưu không gian lưu trữ, gom hàng lẻ hoặc bảo trì kệ. Hệ thống cập nhật vị trí mới và ghi nhận giao dịch TRANSFER vào Sổ Cái Kép.

### 1.2. Primary Actor
- **Nhân viên kho (PDA) / Thủ kho**
...
### 3.1. Main Flow
1. Người dùng mở màn hình (`HandheldLocationTransferView.tsx`).
...
6. Backend kiểm tra Auth và gọi Stored Procedure `api.usp_WMS_INV03_TransferLocation_v1`.
...
### AC-01 — Happy Path
**Given** Người dùng có quyền và dữ liệu ở trạng thái `ON_RACK (Kệ Nguồn)`.  
**When** Gửi yêu cầu thực hiện thao tác.  
**Then** Trả 200 OK, CSDL chuyển sang `ON_RACK (Kệ Đích)`, ghi Sổ Cái Kép và phát âm thanh phản hồi.
...
### 6.1. Frontend — React (`HandheldLocationTransferView.tsx`)
...
- **Stored Procedure:** `api.usp_WMS_INV03_TransferLocation_v1`
...
|---|:---:|:---:|:---:|:---:|---|
| `dbo.tbl_map_nhapkho` | **X** | **X** | **X** | - | Bảng thực thể chính xử lý tồn kho / kiểm kê |
...
sequenceDiagram
    autonumber
    actor User as Nhân viên kho (PDA) / Thủ kho
    participant UI as React UI (HandheldLocationTransferView.tsx)
    ...
    API->>DB: 5. EXEC api.usp_WMS_INV03_TransferLocation_v1 @UserId, @Params
    
    activate DB
    Note over DB: BƯỚC 1: SET XACT_ABORT ON & Kiểm tra quyền
    Note over DB: BƯỚC 2: BEGIN TRANSACTION & Khóa dữ liệu mục tiêu (UPDLOCK)
    Note over DB: BƯỚC 3: Kiểm tra điều kiện nghiệp vụ (ON_RACK (Kệ Nguồn))
...
```mermaid
flowchart TD
    User["Nhân viên kho (PDA) / Thủ kho"]
    ReactUI["React UI (HandheldLocationTransferView.tsx)"]
    BackendAPI["Backend API (.NET 8)"]
    AuthCheck{"Token hợp lệ & Có quyền?"}
    StatusCheck{"Trạng thái hợp lệ (ON_RACK (Kệ Nguồn))?"}
    ...
    ProcessLock -->|"5. Execute api.usp_WMS_INV03_TransferLocation_v1"| DB
    DB -->|"6. COMMIT Transaction: Cập nhật ON_RACK (Kệ Đích)"| DB
...
|---|---|---|---|---|---|
| `TC-01` | Happy Path | User có quyền, dữ liệu hợp lệ | Gửi request xử lý | Trả 200 OK, CSDL chuyển sang `ON_RACK (Kệ Đích)` | `AC-01` |
...
- [x] API Contract & Stored Procedure `api.usp_WMS_INV03_TransferLocation_v1` chuẩn Transaction.
