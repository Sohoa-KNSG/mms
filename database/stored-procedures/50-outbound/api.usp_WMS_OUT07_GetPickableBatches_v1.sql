...
BEGIN
    ...
    DECLARE @MaterialId nvarchar(50), @BravoId nvarchar(50);
    SELECT @MaterialId = LTRIM(RTRIM(line.id_vattu)), @BravoId = NULLIF(LTRIM(RTRIM(line.id_bravo)), N'')
    ...
    SELECT BatchId = batch.id_batch, 
        MaterialId = COALESCE(batch.id_vattu, @MaterialId), 
        BravoId = COALESCE(batch.id_bravo, @BravoId),
        MaterialName = COALESCE(batch.ten_vattu, N'Vật tư'), 
        AvailableQuantity = CONVERT(decimal(18,4), ISNULL(batch.so_luong, 0)),
        Unit = COALESCE(batch.unit, N'Cái'), 
    ...
    WHERE batch.so_luong > 0
      ...
      AND (
          ...
          OR (@BravoId IS NOT NULL AND LTRIM(RTRIM(batch.id_vattu)) = @BravoId)
          OR (LTRIM(RTRIM(batch.id_bravo)) = @MaterialId)
          OR EXISTS (
              SELECT 1 FROM dbo.tbl_dm_vattu v
              WHERE (LTRIM(RTRIM(v.id_vattu)) = @MaterialId OR LTRIM(RTRIM(v.id_bravo)) = @MaterialId 
                     OR (@BravoId IS NOT NULL AND (LTRIM(RTRIM(v.id_vattu)) = @BravoId OR LTRIM(RTRIM(v.id_bravo)) = @BravoId)))
