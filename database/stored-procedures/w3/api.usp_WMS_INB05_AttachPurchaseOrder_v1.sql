CREATE OR ALTER PROCEDURE api.usp_WMS_INB05_AttachPurchaseOrder_v1
    @UserId nvarchar(50),
    @ReceiptId int,
...
BEGIN
    ...
    IF @PurchaseOrder IS NULL OR NOT EXISTS (SELECT 1 FROM @Assignments)
        THROW 51002, N'PO và ánh xạ dòng nhận là bắt buộc.', 1;
    ...
    BEGIN TRY
        ...
        SELECT TOP (1) @CustomerCode = Ma_khach_hang
        FROM dbo.tbl_ChiTietDDH WITH (UPDLOCK, HOLDLOCK)
        WHERE So_DDH_HD = @PurchaseOrder;
        ...
        (
            ...
            THROW 51022, N'Mọi dòng của phiếu phải được ánh xạ vào PO.', 1;
        ...
        (
            ...
            LEFT JOIN dbo.tbl_ChiTietDDH AS po WITH (UPDLOCK, HOLDLOCK)
              ON po.Ma_khoa_chinh = assignment.PurchaseOrderKey
             AND po.So_DDH_HD = @PurchaseOrder AND po.Ma_hang_hoa = line.ma_hang
            OUTER APPLY
            (
            ...
            WHERE line.id_nhanhang IS NULL OR po.Ma_khoa_chinh IS NULL
               ...
               OR COALESCE(NULLIF(line.unit, N''), N'#') <> COALESCE(NULLIF((SELECT unit FROM dbo.tbl_dm_vattu WHERE id_vattu = po.Ma_hang_hoa), N''), N'#')
        )
        ...
        SET ma_po = @PurchaseOrder, khach_hang = @CustomerCode, status_nhap = N'2'
        ...
        SELECT CONVERT(nvarchar(50), ma_phieu), kho, khach_hang, @UserId, time_cre,
            ma_po, id_bv, status_nhap, N'UPDATE_PO', @Now
        FROM dbo.tbl_phieu_nhan_hang WHERE ma_phieu = @ReceiptId;

        COMMIT TRANSACTION;
        SELECT ReceiptId = @ReceiptId, PurchaseOrder = @PurchaseOrder,
