CREATE OR ALTER PROCEDURE api.usp_WMS_INV07_CountLocationBatch_v1
    @UserId nvarchar(50),
...
BEGIN
    ...
    IF @ActualQuantity < 0 OR @LocationCode IS NULL OR @Reason IS NULL THROW 51002, N'Vị trí, số lượng và căn cứ là bắt buộc.', 1;
    ...
    BEGIN TRY
        ...
        WHERE id_batch = @BatchId AND location = @LocationCode AND trang_thai_ton = N'1';
        IF @CurrentQuantity IS NULL THROW 51004, N'Batch không thuộc vị trí được kiểm kê.', 1;
        ...
        INSERT dbo.tbl_transaction (id_batch, id_phieu_trans, nghiep_vu, id_vattu, id_bravo, ten_vattu, so_luong, unit, time_cre, trang_thai)
        VALUES (@BatchId, @DocumentId, N'INV_CNT', @MaterialId, @BravoId, @MaterialName, CONVERT(float, @ActualQuantity), @Unit, @Now, N'2');
        IF @Delta <> 0
            INSERT dbo.tbl_transaction (id_batch, id_phieu_trans, nghiep_vu, id_vattu, id_bravo, ten_vattu, so_luong, unit, time_cre, trang_thai)
        ...
        UPDATE dbo.tbl_batch_inv SET so_luong = CONVERT(float, @ActualQuantity), user_up = LEFT(@UserId, 20), time_up = @Now,
            ma_event_up = CASE WHEN @Delta > 0 THEN N'3' WHEN @Delta < 0 THEN N'4' ELSE ma_event_up END WHERE id_batch = @BatchId;
        COMMIT TRANSACTION;
        SELECT LocationCode = @LocationCode, BatchId = @BatchId, TransactionDocumentId = @DocumentId,
            PreviousQuantity = @CurrentQuantity, ActualQuantity = @ActualQuantity, DifferenceQuantity = @Delta, ChangedAt = @Now;
    END TRY
    BEGIN CATCH
